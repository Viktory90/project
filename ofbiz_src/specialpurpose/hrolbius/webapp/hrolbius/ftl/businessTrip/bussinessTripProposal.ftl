...
$(document).ready(function () {
	var source = {
			...
			datafields:[
			   ...
			   {name: 'fromDate', type:'date', format: "dd-MM-yyyy hh:mm:ss"},
	...
	//console.log("vehicleAdapter.records:", vehicleAdapter.records);
	var souceVeh = {
			localdata: localdata,
			dataType: 'array',
			datafields: [
	...
	jQuery("#VehicleRegis").jqxGrid({
        ...
        columns:[
        	...
        	{text: '${uiLabelMap.DateDeparture}', datafield:'dateDeparture', align: 'center', columntype: 'datetimeinput', columngroup: 'Time', cellsformat: 'dd/MM/yyyy hh:mm tt', width:"16%",
        		createeditor: function (row, column, editor) {
        			//console.log(editor);
             		editor.jqxDateTimeInput({ formatString: 'dd-MM-yyyy hh:mm:ss' });
             	}	
        	},
        	{text: '${uiLabelMap.DateArrival}', datafield:'dateArrival', align: 'center', columntype: 'datetimeinput', columngroup: 'Time', cellsformat: 'dd/MM/yyyy hh:mm tt', width: "16%",
        		 createeditor: function (row, column, editor) {
             		editor.jqxDateTimeInput({ formatString: 'dd-MM-yyyy hh:mm:ss' });
             	}
        	},
	...
	jQuery("#submitForm").click(function(){
		...
		if(flagCheckSubmit){
			jQuery("#messageNotification").jqxNotification({ template: 'warning' , opacity : 1,autoCloseDelay: 5000 ,appendContainer : '#container'});
			jQuery("#messageNotification").jqxNotification("open");
		}else {
			//console.log(cellValueBusDest);	
		...
		if(flagCheckSubmit){
			jQuery("#messageNotification1").jqxNotification({ template: 'warning',opacity : 1,autoCloseDelay: 5000,appendContainer : '#container'});
		...
		if(!flagCheckSubmit){
					...
					success: function(data){
						if(data._EVENT_MESSAGE_){
							jQuery("#notification").html(data._EVENT_MESSAGE_);
							jQuery("#notification").jqxNotification({ template: 'info'});
							jQuery("#notification").jqxNotification("open");
						}else if(data._ERROR_MESSAGE_){
							$("#notification").html("${uiLabelMap.ErrorOccur}: " + data._ERROR_MESSAGE_);
							jQuery("#notification").jqxNotification({ template: 'error'});
							jQuery("#notification").jqxNotification("open");
						}
					},
					error: function(){
