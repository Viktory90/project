...
function renderTableHtml2(data, id, productIdN, uomToIdN){
	...
	table += "</tr></table><table class='table table-bordered dataTable' cellspacing='0'>" +
				...
				"<th class='hidden-phone'>${uiLabelMap.Time}</td>" +
					...
					"<th class='hidden-phone'>${uiLabelMap.InventoriesOnDays}</th>" +
					"<th class='hidden-phone'>${uiLabelMap.Note}</th>" +
	...
	for ( var x in data) {		
		for ( var s in data[x]) {
			...
			if (check) {
				...
				"<td style='text-align: right;'><label id='txtPallet" + loops + "' >0</label></td>" +
	...
	table = "<button type='submit' class='btn btn-primary btn-small' id='btnAddProduct' onclick='btnAddProductClick()' name='submitButton'><i class='icon-ok'></i>${uiLabelMap.CommonSave}</button>";
...
function insertConfigPacking(jsonObject, quantityConvert, isUpdate) {
	...
	var wd = "";
	wd += "<div id='window01'><div>${uiLabelMap.CommonEdit} ${uiLabelMap.ProductCapacity}</div><div>";
	wd += alert;
	wd += "<form><div class='row-fluid'>" +
				"<div class='span12 no-left-margin'>" +
					"<div class='span4'>${uiLabelMap.ProductId}:</div>" +
					"<div class='span8'><div id='productId1' >" + productId + "</div></div>";
				"</div>";
	wd += "<div class='row-fluid'>" +
				"<div class='span12 no-left-margin'>" +
					"<div class='span4'>${uiLabelMap.StorageCapacity}:</div>" +
					"<div class='span8'><input type='text' id='quantityConvert1' name='quantityConvert1' onkeydown='if (event.keyCode == 13) enterPress()'/></div>" +
				"</div>";
	wd += "<div class='row-fluid'>" +
			"<div class='span12 no-left-margin'>" +
				"<div class='span4'>${uiLabelMap.AvailableFromDate}:</div>" +
				"<div class='span8'><div id='availableFromDate1'></div></div>";
			"</div>";
	wd += "<div class='row-fluid'>" +
			"<div class='span12 no-left-margin'>" +
				"<div class='span4'>${uiLabelMap.AvailableThruDate}:</div>" +
				"<div class='span8'><div id='expireDate1'></div></div>" +
			"</div>";
	wd += "<div class='row-fluid'>" +
			"<div class='span12 no-left-margin'>" +
				"<div class='span4'></div>" +
				"<div class='span8'><input style='margin-right: 5px;' type='button' id='alterSave' value='${uiLabelMap.CommonSave}' /><input id='alterCancel' type='button' value='${uiLabelMap.CommonCancel}' /></div>" +
			"</div>";
	wd += "</div></form></div>";
	$("#myImage").html(wd);
	$('#window01').jqxWindow({ height: height, width: 450, isModal: true, modalOpacity: 0.7 });
	$("#availableFromDate1").jqxDateTimeInput();
	$("#expireDate1").jqxDateTimeInput();
