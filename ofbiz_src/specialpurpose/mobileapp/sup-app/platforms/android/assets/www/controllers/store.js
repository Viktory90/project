...
olbius.controller('StoreController', function($rootScope, $scope, $controller, $location, $compile, CustomerService, GPS) {
	...
	var getPage = function(data, page, pagesize) {
		...
		for(var x= 0;x<end ; x++){
			...
			str += obj.groupName;
			str += "</h3><div class='store-row-info'><i class='glyphicon glyphicon-map-marker'></i>&nbsp;<b>{{Map.Address[key]}}:&nbsp;</b>";
			str += obj.address1;
			str += "&nbsp;";
			if (obj.city) {
				str += obj.city;
			}
			str += "</div><div class='store-row-info'>" + "<i class='glyphicon glyphicon-phone-alt'></i>&nbsp;<b>{{Map.Mobile[key]}}:&nbsp;</b>";
			if (obj.phone) {
				str += obj.phone;
			} else {
				str += 0;
