...
public class FirstIntvRating extends Observable{
	
	public  void rateFirstInterview(DispatchContext dpctx,
		...
		if("PASS".equals(resultId)){
			...
			EntityCondition condition2 = EntityCondition.makeCondition("workEffortTypeId", "RECR_REVIEW2");
			...
			if(workEffortList != null && !workEffortList.isEmpty()){
				if(workEffortList.size() == 1){
					...
					throw new Exception("More than one second interview step");
				}
			}else{
				throw new Exception("Second Interview is not exist in this plan");
			}
