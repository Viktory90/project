package com.olbius.delys.product;
...
public class PriceServices {
    	...
    	try {
    		if ("QUOTT_NBD_ACCEPTED".equals(statusId)) {
        		...
        		if (isSend) {
        			...
        			Map<String, Object> resultGetParties = dispatcher.runSync("getPartiesByRootAndRoles", UtilMisc.<String, Object>toMap("partyId", "DSA", "roleTypeIds", listRoleNeedSend, "userLogin", userLogin));
        			...
        			if (listRoleTypeInQuotation != null) {
        				...
        				for (String roleTypeId : roleTypesApply) {
        					listPartyIdFinal.addAll(SalesPartyUtil.getPartiesByRole(delegator, dispatcher, userLogin, roleTypeId, true));
        				}
        			}
            		Map<String, Object> tmpResult = dispatcher.runSync("createNotification", 
            				UtilMisc.<String, Object>toMap("partiesList", listPartyIdFinal, "header", header, "state", state, "action", action, "targetLink", targetLink, "dateTime", dateTime, "ntfType", "ONE", "sendToGroup", sendToGroup, "userLogin", userLogin));
            		if (ServiceUtil.isError(tmpResult)) {
            			return ServiceUtil.returnError(ServiceUtil.getErrorMessage(tmpResult));
                    }
        		}
    ...
    public static Map<String, Object> createProductQuotation(DispatchContext ctx, Map<String, ? extends Object> context) {
    	...
    	String targetLink = "";
    	try {
    		partiesList = SalesPartyUtil.getPersonNBDs(delegator);
    		header = UtilProperties.getMessage(resource, "DAApproveProductQuotation",locale) + " [" + productQuotationId +"]";
    		action = "viewQuotation";
    		targetLink = "productQuotationId="+productQuotationId;
    	} catch (Exception e) {
    ...
    public static Map<String, Object> jqGetListProductQuotaton(DispatchContext ctx, Map<String, Object> context) {
    	...
    	try {
    		...
    		if (security.hasEntityPermission("DELYS_QUOTATION", "_VIEW", userLogin)) {
    			EntityCondition tmpConditon = EntityCondition.makeCondition(listAllConditions,EntityJoinOperator.AND);
    			listQuotation = delegator.findList("ProductQuotation", tmpConditon, null, listSortFields, opts, false);
