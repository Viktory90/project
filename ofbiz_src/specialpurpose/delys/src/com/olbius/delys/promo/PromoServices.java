package com.olbius.delys.promo;
...
public class PromoServices {
	...
	public static Map<String, Object> jqGetListProductPromo(DispatchContext ctx, Map<String, Object> context) {
    	...
    	Map<String,String[]> parameters = (Map<String, String[]>) context.get("parameters");
		try {
			Map<String, Object> resultService = dispatcher.runSync("getProductPromoTypesByChannel", UtilMisc.toMap("salesMethodChannel", "SALES_GT_CHANNEL"));
			if (ServiceUtil.isSuccess(resultService)) {
				List<GenericValue> listPromoType = (List<GenericValue>) resultService.get("listProductPromoType");
				List<String> listPromoTypeId = new ArrayList<String>();
				if (listPromoType != null) {
					listPromoTypeId = EntityUtil.getFieldListFromEntityList(listPromoType, "productPromoTypeId", true);
				}
		...
		if (json != null && json.size() > 0) {
			...
			for (int i = 0; i < json.size(); i++) {
				...
				if ("Y".equals(isApprove)) {
					...
					if (UtilValidate.isNotEmpty(SalesPartyUtil.isSupervisorGTEmployee(userLogin, delegator))) {
						newStatus = "REGPR_SUP_ACCEPTED";
					}
					...
					if (UtilValidate.isNotEmpty(SalesPartyUtil.isAsmGTEmployee(userLogin, delegator))) {
						newStatus = "REGPR_ASM_ACCEPTED";
					}
					...
					if (UtilValidate.isNotEmpty(SalesPartyUtil.isSalesAdminGTEmployee(userLogin, delegator))) {
						newStatus = "REG_PROMO_ACCEPTED";
					}
				}
