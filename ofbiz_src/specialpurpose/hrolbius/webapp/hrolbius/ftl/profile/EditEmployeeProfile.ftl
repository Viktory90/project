...
$(document).ready(function() {
	...
	beforeShow: function (input, inst) {
              ...
              console.log("input:", $(input).position().top , "," , $(input).position().left, ",", $(input).position().bottom);
		...
		jQuery.ajax({
			...
			success: function(data){
				...
				if(data._EVENT_MESSAGE_){
					...
					console.log("<td><button type='button' class='btn btn-small btn-danger btn-mini icon-trash' onclick='deletePersonBackground("+id+",personFamilyBackground_"+ id+" )>"+"${StringUtil.wrapString(uiLabelMap.CommonDelete)}"+"</button></td>");
