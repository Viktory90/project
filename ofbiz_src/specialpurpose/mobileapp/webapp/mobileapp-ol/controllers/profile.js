 ...
 */
olbius.controller('ProfileController', function($rootScope,$location, $scope, $controller,CustomerService,AuthService) {
    ...
    $scope.$on('$viewContentLoaded',function(){
    	// $scope.setHeader($rootScope.Map.ProfileSalesMan[$rootScope.key], "/main", false);
    	$scope.inIt();
    });
    $scope.inIt = function(){
    	$scope.getInfo();
    ...
    $scope.updatePassword = function(){
    	$('#changePasswdForm').trigger('reset');
    	console.log($scope.password + '  ' + $scope.credentials.PASSWORD);
    	
    if($scope.password.CurrentPassword != $scope.credentials.PASSWORD){
    	$scope.password.status.notify = true;
    	console.log($scope.password.status.notify);
    	...
    	$scope.password.status.notity = false;
    			...
    			if(data._ERROR_MESSAGE_){
    				...
    				$scope.credentials.PASSWORD = $scope.password.CurrentPassword;
    ...
    $scope.changeName = function(){
    	...
    	alert(name);
    	if(name.length == 0 ){
    		bootbox.alert($rootScope.Map.NotiPressFullInfo[$rootScope.key]);
    	}
    	...
    	alert($scope.infoSalesMan.firstName);
    	alert($scope.infoSalesMan.middleName);
    	alert($scope.infoSalesMan.lastName);
    			...
    			$scope.infoSalesMan = 
    				{
						...
						middleName : $scope.info.middleName,
    	...
    	BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action: function(dialogRef){
                    ...
                    dialogRef.getModalBody().html('Dialog closes in 1 seconds.');
                    setTimeout(function(){
                        dialogRef.close();
                        $location.path("sync");
                        $scope.$apply();
    ...
    $scope.updateSalesMan = function(){
    	...
    	$scope.edit.checkEdit = false;
    		CustomerService.updateSalesMan($scope.infoSalesMan,$rootScope.showLoading).then(function(data){
    			if(data._ERROR_MESSAGE_){
					   bootbox.alert($rootScope.Map.UpdateInfoError[$rootScope.key]); 				
    			}else  bootbox.alert($rootScope.Map.UpdateInfoSuccess[$rootScope.key]); 
    			$rootScope.hideLoading();
	    	},function(err){
	    		bootbox.alert($rootScope.Map.UpdateInfoError[$rootScope.key]); 	
	    	});
