...
<#assign isSalesAdminManager = Static["com.olbius.delys.util.SalesPartyUtil"].isSalesAdminManagerEmployee(userLogin, delegator)/>
<#assign roleTypes = Static["com.olbius.delys.util.SalesPartyUtil"].getAllSubRoleTypeIds("DELYS_ROLE", delegator)/>
<#if isSalesAdminManager>
	<#assign roleTypesGT = Static["com.olbius.delys.util.SalesPartyUtil"].getAllSubRoleTypeIdsByGroup("SALES_ROLE_GT", delegator)/>
	<#assign roleTypesMT = Static["com.olbius.delys.util.SalesPartyUtil"].getAllSubRoleTypeIdsByGroup("SALES_ROLE_MT", delegator)/>
</#if>
...
<div class="row-fluid">
	<h4 id="step-title" class="header smaller lighter blue span3" style="margin-bottom:0 !important; border-bottom: none; margin-top:3px !important; padding-bottom:0">${uiLabelMap.DACreateQuotationForProduct}</h4>
								...
								<div class="span12">
									<div id="containPartyRoleTypesApply" <#if isAdvanceInterface?string == "false">style="display:none"</#if>>
										<div>
											...
											<span class="help-inline" title="${uiLabelMap.DARoleIdOfPartyGroupApplyPriceToCustomer}" data-rel="tooltip" data-placement="top" style="vertical-align: top">(1)</span>
										</div>
										...
										<div>
											...
											<span class="help-inline" title="${uiLabelMap.DAPartyApplyPriceToMarket}" data-rel="tooltip" data-placement="top" style="vertical-align: top">(2)</span>
										</div>
										...
										<div style="margin-top:5px">
											...
											<span class="help-inline" title="${uiLabelMap.DAPartyIdApplyPriceToCustomer}" data-rel="tooltip" data-placement="top" style="vertical-align: top">(3)</span>
										</div>
									...
									<input type="text" id="partyRoleTypesApplyDefault" name="partyRoleTypesApplyDafult" value="<#if selectedSalesChannel == "SALES_MT_CHANNEL">DELYS_CUSTOMER_MT<#else>DELYS_DISTRIBUTOR</#if>" readonly="readonly" class="span12" style="width:220px; <#if isAdvanceInterface?string == "true">display:none</#if>"/>
								</div>
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls">
								<div class="span12">
									...
									<div style="display:inline-block; margin-left:25px; vertical-align:top; margin-top:-5px">
										<button type="button" id="copyToMarket" title="${uiLabelMap.DACopyPriceDistToPriceMarket}" data-rel="tooltip" data-placement="top">${uiLabelMap.DACopy}</button>
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	function loadMtInterface() {
		$("#partyRoleTypesApplyDefault").val("DELYS_CUSTOMER_MT");
	...
	$(document).ready(function () {
    	...
    	$('#percentagePriceToConsumer').on('change', function (event) {
	 	...
	 	$('#fuelux-wizard').ace_wizard().on('change' , function(e, info){
			if ((info.step == 1) && (info.direction == "next")) {
				...
				$("#strProductQuotationId").html($("#productQuotationId").val());
				$("#strQuotationName").html($("#quotationName").val());
				$("#strDescription").html($("#description").val());
				$("#strSalesChannel").html($("#salesChannel").val());
				$("#strCurrencyUomId").html($("#currencyUomId").val());
				$("#strFromDate").html($("#fromDate").val());
				$("#strThruDate").html($("#thruDate").val());
				if ($('#advanceInterface').val()) {
			...
			bootbox.confirm("${uiLabelMap.DAAreYouSureCreate}", function(result) {
				if(result) {
		            ...
		            var hiddenField = document.createElement("input");
		            hiddenField.setAttribute("type", "hidden");
		            hiddenField.setAttribute("name", "advanceInterface");
		            hiddenField.setAttribute("value", advanceInterface);
		            formSend.appendChild(hiddenField);
		...
		$('#createQuotation').jqxValidator({
        	rules: [
				...
				{input: '#fromDate', message: '${StringUtil.wrapString(uiLabelMap.HrolbiusRequiredValueGreatherToDay)}', action: 'blur', rule: 
					function (input, commit) {
						var now = new Date();
