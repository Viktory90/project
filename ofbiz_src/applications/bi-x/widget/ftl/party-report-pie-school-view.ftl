...
//            },
        ...
        popup._addDropDownList("school_type", "Statistics", ["EDU_SYS", "CLASSIFICATION", "SCHOOL", "STUDY_MODE", "MAJOR"], 0);

        popup._addOK(function () {
            popup._close();
            loadSchool(popup._getParty(), $("#school_type").val());
        });
    });

    function loadSchool(group, type) {
        ...
        jQuery.ajax({
            ...
            data: {
                'group' : group,
                //EDU_SYS, CLASSIFICATION, SCHOOL, STUDY_MODE, MAJOR
                'type': type
            },
