...
<#macro jqGridMinimumLib>
	...
	<!--<script type="text/javascript" src="/delys/images/js/customDelys.js"></script>-->
</#macro>
...
<#macro jqGrid url columnlist dataField initrowdetailsDetail="" isShowTitleProperty="true" customTitleProperties="" columngrouplist="" updaterow="" addColumns="" deleterow="" addrow="" updateUrl="" createUrl="" removeUrl="" showtoolbar="true" addmultiplerows="false" updateMulUrl="" updatemultiplerows="" updateoffline="false"
			   ...
			   primaryColumn="ID" editable="false" editColumns="" id="jqxgrid" dataType="json" filterable="true" filtersimplemode="true" viewSize="20" viewIndex="0" width="500" height="" autorowheight="false"
			   pageable="true" columnsresize="true" columnsreorder="true" sortable="true" defaultSortColumn="" autoheight="true" currencySymbol="â‚«" selectionmode="singlerow" addrefresh="false" 
			   ...
			   editmode="selectedrow" jqGridMinimumLibEnable="true" otherParams="" sortdirection="asc" altrows="false" sourceId="Id" rowselectfunction="" rowunselectfunction="" idExisted="false"
			   ...
			   usecurrencyfunction="true" deletesuccessfunction="" autoload="true" statusbarheight="50" dropdownlist="false" ddlSource="" displayMember="" valueMember="" sendEmail="false" changeState="false" bindresize="true" customcontrol1="" customcontrol2="" customcontrol3=""  showlist="true" 
			   extraUrl="" offmode="false" allGridMenu="false" rowsheight="30" functionAfterUpdate="" functionAfterAddRow="" customLoadFunction="false" rowdetailsheight="200" 
			   readyFunction="" rowdetailstemplateAdvance="" deletelocal="false" enablemousewheel="true">
   <#if jqGridMinimumLibEnable=="true">
	...
	<script type="text/javascript">
		...
		var deleteRow = function(url, data, commit){
			$.ajax({
                type: "POST",
                url: url,
                data:  data,
                ...
                success: function (data, status, xhr) {
                    if(data.responseMessage == "error"){
                    	...
                    	$("#jqxNotification${id}").text(data.errorMessage);
                    	...
                    	$("#jqxNotification${id}").text("${StringUtil.wrapString(uiLabelMap.wgdeletesuccess)}");
                    	$("#jqxNotification${id}").jqxNotification("open");
		...
		var updateRow = function(url, data, commit){
			$.ajax({
                type: "POST",                        
                url: url,
                data: data,
                ...
                success: function (data, status, xhr) {
                    ...
                    if(data.responseMessage == "error"){
                    	...
                    	$('#container${id}').empty();
                    	$('#jqxNotification${id}').jqxNotification({ template: 'info'});
                    	$("#jqxNotification${id}").text("${StringUtil.wrapString(uiLabelMap.wgupdatesuccess)}");
                    	$("#jqxNotification${id}").jqxNotification("open");
		...
		var checkDataUpdate = function(data1, data2){
			...
			for(var x in data1){
				...
				for(var y in data2){
					...
					var j = 0;
					for(var z in obj2){
						console.log(obj[z].value, obj2[z]);
		...
		<#else>
            ...
            var source =
            {
                ...
                <#if editable=="true" || updaterow=="true" || doubleClick=="true" || updatemultiplerows=="true" || editColumns!="">
                updaterow: function (rowid, rowdata, commit, a, b, c) {
					<#if offmode=="true">
						commit(true);
						return;
					</#if>
                	...
                	{
	                	var columnList = '${editColumns}'.toString();
	                	var arrKeysData = columnList.split(";");
	                	var rowidlength = 1;
		                			...
		                			if(rowdata[j][tmpStr]==null){	                				
		                				if(arrKeysData[i].indexOf("[") > -1){
		                					if(arrKeysData[i].indexOf(".Timestamp)") > -1){
			                					var tmpdate = new Date(arrKeysData[i].substring(arrKeysData[i].indexOf('[') + 1,arrKeysData[i].length - 1));
											    var tmps = tmpdate.getTime() + "";
				                				columnValues += tmps + "#;";
			                				}else{
		                				...
		                				if(arrKeysData[i].indexOf(".Timestamp)") > -1){
		                					var tmpdate = new Date(rowdata[j][tmpStr]);
										    var tmps = tmpdate.getTime() + "";
			                				columnValues += tmps + "#;";
		                				}else{
                	...
                	if (urlStatus && !(/^\s*$/.test(urlStatus))) {
                		updateRow(urlStatus, data, commit);
                	}
                	<#else>
                ...
                addrow: function (rowid, rowdata, position, commit) {                
                	...
                	var tmpAddclm = "";
                	var tmpKey = addColumns[0];
                	if(addColumns[0].indexOf('(') > -1){
                		tmpKey = addColumns[0].substring(0,addColumns[0].indexOf('('));
                	}
                	if(addColumns[0].indexOf('[') > -1){
                		if(addColumns[0].indexOf(".Timestamp)") > -1 || addColumns[0].indexOf(".Date)") > -1){
        					var tmpdate = new Date(addColumns[0].substring(addColumns[0].indexOf('[') + 1,addColumns[0].length - 1));
						    var tmps = isNaN(tmpdate.getTime()) ? "" : tmpdate.getTime() ;
            				data = "columnValues" + "=" + tmps;
        				}else{
        					data = "columnValues" + "=" + addColumns[0].substring(addColumns[0].indexOf('[') + 1,addColumns[0].length - 1);
        				}
                	}else{
                		if(addColumns[0].indexOf(".Timestamp)") > -1 || addColumns[0].indexOf(".Date)") > -1){
        					var tmpdate = new Date(rowdata[tmpKey]);
						    var tmps = isNaN(tmpdate.getTime()) ? "" : tmpdate.getTime() ;
            				data = "columnValues" + "=" + tmps;
        				}else{
        					data = "columnValues" + "=" + rowdata[tmpKey];
        				}
                	}
                	if(addColumns[0].indexOf('[') > -1){
                		tmpAddclm = addColumns[0].substring(0,addColumns[0].indexOf('['));;
                	}else{
                		tmpAddclm = addColumns[0];
                	}
                	for(i=1;i<addColumns.length;i++){
                		tmpKey = addColumns[i];
                		...
                		if(addColumns[i].indexOf('[') > -1){
                			if(addColumns[i].indexOf(".Timestamp)") > -1 || addColumns[i].indexOf(".Date)") > -1){
                				var tmpdate = new Date(addColumns[i].substring(addColumns[i].indexOf('[') + 1,addColumns[i].length - 1));
							    var tmps = isNaN(tmpdate.getTime()) ? "" : tmpdate.getTime() ;
	            				data += "#;" + tmps
                			}else{
                				data += "#;" + addColumns[i].substring(addColumns[i].indexOf('[') + 1,addColumns[i].length - 1);
                			}
	                	}else{
	                		if(addColumns[i].indexOf(".Timestamp)") > -1 || addColumns[i].indexOf(".Date)") > -1){
	        					var tmpdate = new Date(rowdata[tmpKey]);
                		...
                		if(addColumns[i].indexOf('[') > -1){
	                		tmpAddclm += ";" + addColumns[i].substring(0,addColumns[i].indexOf('['));;
	                	}else{
	                		tmpAddclm += ";" + addColumns[i];
	                	}
                	} 
                	...
                	$.ajax({
                        ...
                        success: function (data, status, xhr) {
                            if(data.responseMessage == 'error'){
                            	...
                            	$('#container${id}').empty();
                            	$('#jqxNotification${id}').jqxNotification({ template: 'info'});
                            	$("#jqxNotification${id}").text("${StringUtil.wrapString(uiLabelMap.wgaddsuccess)}");
                            	$("#jqxNotification${id}").jqxNotification("open");
                            	<#if addrefresh=="true">
                            		$('#${id}').jqxGrid('updatebounddata');
                            	</#if>
                ...
                deleterow: function (rowid, commit) {
                	var dataRecord = $('#${id}').jqxGrid('getrowdata', rowid);
                	...
                	for(i=1; i < dcl.length;i++){
                		if(dcl[i].indexOf('[') > -1){
		            		...
		            		if(dcl[i].indexOf("(") > -1){
		                		...
		                		var tmpD = new Date(dataRecord[tmpStr]);
                				tmpStr = tmpD.getTime() + "";
                				console.log(tmpStr.substring(0, tmpStr.length - 3));
                				tmpValue += "#;" + tmpStr;
            				}else{
            					data += ";" + dcl[i];
        						tmpValue += "#;" + dataRecord[tmpStr];
                	...
                	data += tmpValue;
                	deleteRow("${removeUrl}", data, commit);
                	<#else>
                		var local = {};
            ...
            $("#${id}").jqxGrid(
            {
                ...
                rendertoolbar: function (toolbar) {
                    ...
                    <#if titleProperty?has_content && isShowTitleProperty=="true" && customTitleProperties == "">
                    	<@renderJqxTitle titlePropertyTmp=titleProperty/>
                    <#elseif isShowTitleProperty=="true" && customTitleProperties != "">
                    	<@renderJqxTitle titlePropertyTmp=customTitleProperties/>
                	<#else>
                    ...
                    <#if showlist != "false">
                    	(function(){
                    		<#if showlist == "true">
	                    		var allFields = $("#${id}").jqxGrid('columns').records;
	                    		var strSList = [];
		                    ...
		                    $("#showSL${id}").jqxDropDownList({ checkboxes: true, source: listSource, displayMember: "label", valueMember: "value"});
		                    $("#frozenSL${id}").jqxDropDownList({ checkboxes: true, source: listSource, displayMember: "label", valueMember: "value"});
		                    $("#frozenSL${id}").jqxDropDownList('uncheckAll'); 
		                    container.append('<button id="btngridsetting" class="btn btn-mini" onclick="openJqxConfigWindow${id}();"><i class="fa-cogs"></i></button>');
		                    // init window
		                    $("#jqxconfig${id}").jqxWindow({
		    	                width: 390, resizable: false,  isModal: true, autoOpen: false, cancelButton: $("#configCancel${id}"), modalOpacity: 0.7           
		    	            });
		                    $("#configCancel${id}").jqxButton();
		            	    $("#configSave${id}").jqxButton();
		                    ...
		                    obj.on('click', function () { 
		                    	...
		                    	<#if addType=="popup">
		                    		...
		                    		<#if alternativeAddPopup=="">
		        						...
		        						$("#popupWindow").jqxWindow({ position: { x: (wtmp.outerWidth - tmpwidth)/2, y: pageYOffset + 80 } });
		                        	...
		                        	<#else>
		                        		...
		                        		$("#${alternativeAddPopup}").jqxWindow({ position: { x: (wtmp.outerWidth - tmpwidth)/2, y: pageYOffset + 80 } });
		                        ...
		                        if (selectedrowindex >= 0 && selectedrowindex < rowscount) {                           
		                			...
		                			$("#dialog-delete${id}").dialog({
								      ...
								      buttons: {
										...
										var commit = $("#${id}").jqxGrid('deleterow', selectedrowindex);				                 				                 					          						          
								        },
                    ...
                    <#if updateoffline=="true">
	                	(function(){
		                    ...
		                    obj.on('click', function () {
		                    	...
		                    	updateRow(urlStatus, {data:dataupdate});
		                    	deleteRow("${removeUrl}",{data:datadelete});
		                    	$('#${id}').jqxGrid('updatebounddata');
		                    });                     		
	                	})();
	                </#if>		
	            ...
	            $(window).bind('resize', function() {
	            	...
	            	if($('#sidebar').css("display") != "none"){
	            		...
	            		if($('#sidebar').hasClass("menu-min") != null){
							$('#${id}').jqxGrid({ width: $(window).width() - $('#sidebar').width() - 40 });            			
	            		}else{
	            			$('#${id}').jqxGrid({ width: $(window).width() - $('#sidebar').width() - 40 });
	            		}
	            		// END COMMENT
	            	}else{
	            		$('#${id}').jqxGrid({ width: $(window).width() - 40 });
	            	}
	            	$("#jqxNotification${id}").jqxNotification({ width: $('#${id}').jqxGrid('width'), appendContainer: "#container${id}", opacity: 0.9, autoClose: false, template: "info" });
	            	$("#jqxNotification").jqxNotification({ width: $('#${id}').jqxGrid('width'), appendContainer: "#container", opacity: 0.9, autoClose: false, template: "info" });
				}).trigger('resize');
            ...
            <#if updatemultiplerows=="true">
	          	$("#popupModifyWindow").jqxWindow({
	                width: 450, resizable: false,  isModal: true, autoOpen: false, cancelButton: $("#CancelModify"), modalOpacity: 0.7           
	            });
    ...
    <#if showlist != "">
    	<div id="jqxconfig${id}" style="display:none;">
    		...
    		<div style="overflow: hidden;" id="windowContent">
    			<div style="margin-top:10px;display:block;margin-left:15px;" class="span6">
	    			<div style="width:25%;float:left;margin-top:6px;">${uiLabelMap.cldisplay}</div>
		    		<div style="width:50%;float:left;" id="showSL${id}"></div>
	    		</div>
	    		<div style="margin-top:10px;display:block;margin-left:15px;" class="span6">
		    		<div style="width:25%;float:left;margin-top:6px;">${uiLabelMap.clpin}</div>
		    		<div style="width:50%;float:left;" id="frozenSL${id}"></div>
	    		</div>
	    		<div style="margin-top:10px;margin-bottom:10px;display:block;margin-left:140px;" class="span6">
	    			<input type="button" id="configSave${id}" class="btn btn-mini btn-primary" value="✔  ${uiLabelMap.CommonSave}" />
	    			<input id="configCancel${id}" type="button" class="btn btn-mini btn-primary" value="✖  ${uiLabelMap.CommonCancel}" />
	    		</div>
    		</div>
    	</div>
    	<script type="text/javascript">
...
<#macro renderDateTimeFilterType arrayName="dataDateTimeFilterType">
    ...
    row["description"] = "Nhỏ hơn";
    ${arrayName}[4] = row;
    row = {};
    row["datetimeFilterType"] = "LESS_THAN_OR_EQUAL";
    row["description"] = "Nhỏ hơn hoặc bằng";
    ${arrayName}[5] = row;
    row = {};
    row["datetimeFilterType"] = "GREATER_THAN";
    row["description"] = "Lớn hơn";
    ${arrayName}[6] = row;
    row = {};
    row["datetimeFilterType"] = "GREATER_THAN_OR_EQUAL";
    row["description"] = "Lớn hơn hoặc bằng";
    ${arrayName}[7] = row;
</#macro>
<#macro renderJqxTitle titlePropertyTmp>
	<#if titlePropertyTmp?contains("-")>
		var container = $("<div id='toolbarcontainer' class='widget-header'><h4>"
			...
			+ "</h4></div>");
	<#else>
		var container = $("<div id='toolbarcontainer' class='widget-header'><h4>" + "${StringUtil.wrapString(uiLabelMap[titlePropertyTmp])}" + "</h4></div>");
	</#if>
</#macro>
