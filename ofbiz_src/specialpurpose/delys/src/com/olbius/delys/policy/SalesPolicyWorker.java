 ...
 * under the License.
 *******************************************************************************/
package com.olbius.delys.policy;
...
public class SalesPolicyWorker {
    ...
    public static List<EntitySalesCommission> getSalesCommissionList(DispatchContext ctx, String roleTypeId, String customTimePeriodId, Timestamp fromDate, Timestamp thruDate, Locale locale) {
		List<EntitySalesCommission> listSalesCommission = new ArrayList<EntitySalesCommission>();
							...
							for (GenericValue salesStatementItem : listSalesStatement) {
								...
								if (listSalesPolicy != null) {
									...
									if (listSalesPolicyApply.size() > 0) {
										EntitySalesCommission commissionItem = null;
										try {
											commissionItem = runSalesPolicies(salesStatementItem, listSalesPolicyApply, delegator, fromDate, thruDate, nowTimestamp, true);
    ...
    protected static EntitySalesCommission runSalesPolicies(GenericValue salesStatement, List<GenericValue> salesPolicyList, Delegator delegator, Timestamp fromDate, Timestamp thruDate, Timestamp nowTimestamp, boolean isolatedTestRun) throws GeneralException {
        ...
        EntitySalesCommission salesCommissionEntity = new EntitySalesCommission();
        List<EntitySalesCommissionAdjustment> salesCommissionAdjustmentEntity = new ArrayList<EntitySalesCommissionAdjustment>();
        try {
            ...
            while (cartChanged) {
                ...
                for (GenericValue salesPolicy : salesPolicyList) {
                    ...
                    if (UtilValidate.isNotEmpty(salesPolicyRules)) {
                        ...
                        try {
                        	List<EntitySalesCommissionAdjustment> listSalesCommissionAdj = runSalesPolicyRules(salesStatement, cartChanged, salesPolicy, salesPolicyRules, delegator, nowTimestamp);
        ...
        salesCommissionEntity.setSalesStatementId(salesStatement.getString("salesId"));
        salesCommissionEntity.setPartyId(salesStatement.getString("internalPartyId"));
        salesCommissionEntity.setFromDate(fromDate);
        salesCommissionEntity.setThruDate(thruDate);
        salesCommissionEntity.setListSalesCommissionsAdj(salesCommissionAdjustmentEntity);
        return salesCommissionEntity;
    }
    
    protected static List<EntitySalesCommissionAdjustment> runSalesPolicyRules(GenericValue salesStatement, boolean cartChanged, GenericValue salesPolicy, List<GenericValue> salesPolicyRules, Delegator delegator, Timestamp nowTimestamp) throws GenericEntityException, UseLimitException {
        /*String salesPolicyId = salesPolicy.getString("salesPolicyId");
        BigDecimal totalDiscountAmount = BigDecimal.ZERO;
        BigDecimal quantityLeftInActions = BigDecimal.ZERO;*/
    	@SuppressWarnings("unused")
		boolean promoUsed = false;
        List<EntitySalesCommissionAdjustment> returnValue = new ArrayList<EntitySalesCommissionAdjustment>();
        ...
        while (policyRulesIter != null && policyRulesIter.hasNext()) {
            ...
            List<GenericValue> salesPolicyConds = delegator.findByAnd("SalesPolicyCond", UtilMisc.toMap("salesPolicyId", salesPolicy.get("salesPolicyId")), UtilMisc.toList("salesPolicyCondSeqId"), true);
            ...
            if (performActions) {
                ...
                // List<EntitySalesCommissionAdjustment> listSalesCommissionAdj = new ArrayList<EntitySalesCommissionAdjustment>();
                while (salesPolicyActionIter != null && salesPolicyActionIter.hasNext()) {
                    GenericValue salesPolicyAction = salesPolicyActionIter.next();
                    try {
                    	EntitySalesCommissionAdjustment salesCommissionAdjItem = performAction(salesPolicyAction, salesStatement, delegator, nowTimestamp);
    ...
    protected static EntitySalesCommissionAdjustment performAction(GenericValue salesPolicyAction, GenericValue salesStatement, Delegator delegator, Timestamp nowTimestamp) throws GenericEntityException, CartItemModifyException {
    	EntitySalesCommissionAdjustment actionResultInfo = new EntitySalesCommissionAdjustment();
    ...
    protected static boolean checkCondition(GenericValue salesPolicyCond, GenericValue salesStatement, Delegator delegator, Timestamp nowTimestamp) throws GenericEntityException {
        ...
        if ("SPIP_PC_TURN_OVER".equals(inputParamEnumId)) {
            ...
            BigDecimal percentNeeded = BigDecimal.ZERO;
            BigDecimal percentValue = BigDecimal.ZERO;
            if (UtilValidate.isNotEmpty(condValue)) {
                percentNeeded = new BigDecimal(condValue);
            ...
            while (percentNeeded.compareTo(BigDecimal.ZERO) > 0 && listSalesStatementIter.hasNext()) {
                ...
                if (passedItemConds && !"".equals(productId) && productIds.contains(productId)) {
                	...
                	percentValue = amountActual.divide(amount, generalRounding).multiply(BigDecimal.valueOf(100));
                }
            }
            compareBase = percentValue.compareTo(percentNeeded);
    ...
    public static void performAction(EntitySalesCommissionAdjustment actionResultInfo, GenericValue salesPolicyAction, GenericValue salesStatement, Delegator delegator, Timestamp nowTimestamp) throws GenericEntityException, CartItemModifyException {
