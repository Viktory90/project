...
<#if orderHeader?exists && orderHeader?has_content>
	<i class="icon-angle-right"></i> ${uiLabelMap.DAId}: <a href="<@ofbizUrl>orderView?orderId=${orderId}</@ofbizUrl>" title="<#if orderHeader.orderName?has_content>${orderHeader.orderName?if_exists}</#if>">${orderId}</a>
</#if>
