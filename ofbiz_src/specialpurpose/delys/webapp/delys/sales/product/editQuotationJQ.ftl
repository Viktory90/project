	...
	<div id="alterpopupWindow">
		...
		<div class="form-horizontal basic-custom-form form-size-small">
	    	...
	    	<div class="row-fluid" style="margin-top:20px">
				...
				<button type="button" id="alterSave4" class="btn btn-primary btn-small pull-right" style="padding: 5px 10px"><i class="icon-ok open-sans"></i>${uiLabelMap.CommonSave}</button>
	    	</div>
	    </div>
	</div>		
</form>	
...
<script type="text/javascript">
	...
	// ==============================================================================================================
	    ...
	    $("#alterpopupWindow").jqxWindow({width: 500, height: 330, resizable: false, isModal: true, autoOpen: false, cancelButton: $("#alterCancel4"), modalOpacity: 0.7, theme:theme});
	    ...
	    $('#alterpopupWindowform').jqxValidator({
	        rules: [
	           	{input: '#priceToDist', message: '${uiLabelMap.CommonRequired}. ${uiLabelMap.DANotValidateDataOnlyNumber}', action: 'keyup, blur', 
	           		rule: function (input) {
	           			var value = $(input).val();
           		...
           		<#if quotationSelected.salesChannel != "SALES_MT_CHANNEL">
           		{input: '#priceToMarket', message: '${uiLabelMap.DANotValidateDataOnlyNumber}', action: 'keyup, blur', 
	           		rule: function (input) {
	           			var value = $(input).val();
           		...
           		{input: '#priceToConsumer', message: '${uiLabelMap.DANotValidateDataOnlyNumber}', action: 'keyup, blur', 
	           		rule: function (input) {
	           			var value = $(input).val();
	           			if (isNaN(value)) return false;
	           			else return true;
