...
<div id="screenlet_1_col" class="widget-body-inner">
					...
					<div class="controls">
						<span class="ui-widget">
							<select name="department" id="getInforDepartment_department" size="1">
                                <#if listDepartment?exists>
                                    <#list listDepartment as item>
                                        <option value="${item}"> <#if mapName?exists>${mapName.get(item)}</#if></option>
                                    </#list>
                                </#if>
							</select>
						</span>
	...
	<#assign columnlist=columnlist+",{text:'${uiLabelMap.totalAmount}'+' (+VAT)',datafield:'grandTotal',width:150,cellsalign: 'center', editable: false,filterable:false,cellsrenderer: function(row, column, value) {
