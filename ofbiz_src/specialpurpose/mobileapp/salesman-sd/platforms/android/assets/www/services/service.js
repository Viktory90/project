...
olbius.factory('CustomerService', function($http, $location) {
	...
	srv.getInfoSalesMan = function(beforeSend) {
		return $http({
			url : baseUrl + 'getInfoSalesMan',
			dataType : "json",
			method : "POST",
			transformRequest : function(obj) {
				return transformRequest(obj, beforeSend);
	...
	srv.updateSalesMan = function(data,beforeSend) {
		console.log('updateeeeee');
		return $http({
			url : baseUrl + 'updateSalesMan',
			dataType : "json",
			method : "POST",
			data : {
...
olbius.factory('OrderService', function($http, $location,$rootScope) {
	var srv = {};
	...
	/* get orders */
	srv.getList = function(beforeSend, page, size,month) {
		return $http({
	...
	srv.submitOrder = function(customerId,products,positionCreateOrder,beforeSend ,timeout) {
		var latitude = positionCreateOrder.latitude;
		var longtitude = positionCreateOrder.longitude;
		return $http({
...
olbius.factory('ProductService', function($http, $location) {
	...
	srv.accumulateRegister = function(info){
		console.log('nam' + JSON.stringify(info));
		return $http({
...
olbius.factory('GPS', function($cordovaGeolocation, $route) {
	...
	srv.getCurrentLocation = function(scope, isWatch) {
		...
		$cordovaGeolocation.getCurrentPosition().then(function(position) {
			console.log("current get: " + JSON.stringify(scope.currentLocation));
