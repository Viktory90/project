...
<#assign columnlist = "{text: '${uiLabelMap.EmployeeId}', datafield: 'partyId',  editable: false,  cellsalign: 'left', width: 110},
						{text: '${uiLabelMap.EmployeeName}', datafield: 'partyName', editable: false, cellsalign: 'left', width: 120},
...
<script>
		...
		$("#updateEmplOvertimeWorking").click(function(event){
			if(partyId){
				$.ajax({
					...
					type: 'POST',
					success: function(data){
						renderJqx($("#jqxTimeRegister"), partyId);
					}
				});
			}
		});
	...
	function showTimeRegister(id){
		...
		renderJqx($("#jqxTimeRegister"), id);
	}
	
	function renderJqx(obj, partyId){
		var urlStr = "getWorkOvertimeRegistration";
		...
		$.ajax({
			...
			data: {partyId: partyId, month: month, year: year},
			success: function(data){
				timeRegisSource.localdata = data.data;
