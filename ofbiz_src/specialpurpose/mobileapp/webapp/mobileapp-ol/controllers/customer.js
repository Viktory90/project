...
olbius.controller('CustomerController', function($rootScope, $scope, $controller, $location, $window, CustomerService, LoadRoadService) {
	...
	$scope.$on('$viewContentLoaded', function() {
		$scope.setHeader("Khách hàng", "/main", false);
		$scope.init();
		$scope.scroll();
	});
//	$scope.testSynchronize = function(){
//		var str = [1,2,3,4];
...
//	};
	...
	$scope.getList = function(refresh, close) {
		var partyId = "";
		var key = "customers";
		CustomerService.getAll($scope.currentPage, 30, refresh).then(function(data) {
