...
public class ProductPromoWorker {
    ...
    public static List<GenericValue> getProductStorePromotions(ShoppingCart cart, Timestamp nowTimestamp, LocalDispatcher dispatcher) {
        ...
        try {
        	// TODOCHANGE change get promoProgram list apply from ProductStorePromoAppl to ProductStorePromoApplFilter
            // loop through promotions and get a list of all of the rules...
            //Old List<GenericValue> productStorePromoApplsList = productStore.getRelated("ProductStorePromoAppl", null, UtilMisc.toList("sequenceNum"), true);
        	List<GenericValue> productStorePromoApplsList = delegator.findByAnd("ProductStorePromoApplFilter", UtilMisc.toMap("productStoreId", productStore.get("productStoreId")), UtilMisc.toList("sequenceNum"), false);
        	productStorePromoApplsList = EntityUtil.filterByDate(productStorePromoApplsList, nowTimestamp);
        	
        	List<String> listRoleTypeOfCustomer = FastList.newInstance();
            ...
            while (prodCatalogPromoAppls != null && prodCatalogPromoAppls.hasNext()) {
                ...
                if (UtilValidate.isNotEmpty(prodCatalogPromoAppl.getString("productPromoTypeId"))) {
                	if (listRoleTypeOfCustomer == null || listRoleTypeOfCustomer.size() <= 0) {
                		// TODOCHANGE add code: get list role type of customer
                    	String customerId = cart.getOrderPartyId();
                    	if (UtilValidate.isNotEmpty(customerId)) {
                    		List<GenericValue> listRoleTypeOfCustomerGeneric = delegator.findByAnd("RoleTypeAndParty", UtilMisc.toMap("partyId", customerId), null, true);
                    		Iterator<GenericValue> listRoleTypeOfCustomerInterator = UtilMisc.toIterator(listRoleTypeOfCustomerGeneric);
                    		while (listRoleTypeOfCustomerInterator != null && listRoleTypeOfCustomerInterator.hasNext()) {
                                GenericValue item = listRoleTypeOfCustomerInterator.next();
                                if (UtilValidate.isNotEmpty(item.getString("roleTypeId"))) {
                                	listRoleTypeOfCustomer.add(item.getString("roleTypeId"));
                                }
                                if (UtilValidate.isNotEmpty(item.getString("parentTypeId"))) {
                                	listRoleTypeOfCustomer.add(item.getString("parentTypeId"));
                                }
                    		}
                    	}
                	}
                	...
                	if (listRoleTypeOfCustomer != null && listRoleTypeOfCustomer.size() > 0) {
                        ...
                        List<GenericValue> listPromoRoleTypeApply = delegator.findByAnd("ProductPromoRoleTypeApply", UtilMisc.toMap("productPromoId", prodCatalogPromoAppl.getString("productPromoId")), null, true);
                        for (GenericValue itemRoleType : listPromoRoleTypeApply) {
                        	if (UtilValidate.isNotEmpty(itemRoleType.getString("roleTypeId"))) {
                        		if (listRoleTypeOfCustomer.contains(itemRoleType.getString("roleTypeId"))) {
                        			flagApply = true;
                        			break;
                        		}
                        	}
