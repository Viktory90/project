            ...
            for (var h = 0; h < o; h++) {
                ...
                if (k) {
                    if (e.filtertype == "custom" && e.createfilterwidget) {
                        ...
                        if (e.filterable) {
                        	if(e.datatype == "date" && this._filterrowcache[e.datafield]){
                        		g = true;
            ...
            switch (C.filtertype) {
                ...
                case "olbiusdropgrid":
                	if (this.host.jqxDropDownButton) {
                         ...
                         var t = a("<input id='input" + tmpColName + "' style='height:21px;'/>"); //<div id='jqxgrid" + C.datafield + "'></div>
                         ...
                         l = k;
                         t.appendTo(d);
                         //var str = '#jqxgrid' + C.datafield;
                         //$(str).jqxGrid();
                         //$(str).jqxGrid('showloadelement');
                         //t.jqxDropDownButton();
                         /*$('#jqxButton22').jqxButton();
                         $('#jqxButton22').on('click',function(){
                        	 var stmp = '#jqxwindow' + tmpColName + 'key';
                        	 $(stmp).val("input" + tmpColName);
                        	 console.log($(stmp).val());
                        	 console.log($(stmp));
                         ...
                         t.keydown(function(K) {
                             ...
                             if (t[0]._writeTimer) {
                                 clearTimeout(t[0]._writeTimer)
                             }
                             t[0]._writeTimer = setTimeout(function() {
                                 if (!G._loading) {
                                     if (G["_oldWriteText" + t[0].id] != t.val()) {
                                         G._applyfilterfromfilterrow();
                                         G["_oldWriteText" + t[0].id] = t.val()
                                     }
                                 }
                             }, C.filterdelay);
                             G.focusedfilter = t
