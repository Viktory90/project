 ...
 */
olbius.controller('MarkExhibitedSupController', function($rootScope, $scope, $controller,ProductService ,CustomerService) {
	$.extend(this, $controller('BaseController', {
		$scope : $scope
	}));
	$scope.$on('$viewContentLoaded',function(){
		$scope.setHeader("Trưng bày[SUP]", "/sale", false);
		$scope.inIt();
	});
	$scope.date = {};
	$scope.listMarked = [];
	...
	$scope.changeTab2 = function(){
		...
		// }else bootbox.alert("Hãy chọn cửa hàng cần chấm điểm Trưng bày");foRegister = new Array();
		};
