...
public class GlobalSettingServices {
	...
	public Map<String, Object> createHolidayInYear(DispatchContext dctx, Map<String, Object> context) throws GenericEntityException{
		Delegator delegator = dctx.getDelegator();
...
//		String holidayTypeIds=(String)context.get("holidayTypeIds");
		fromDate = UtilDateTime.getDayStart(fromDate);
		thruDate = UtilDateTime.getDayEnd(thruDate, timeZone, locale);
		String holidayId = delegator.getNextSeqId("Holidays");
//		if(fromDate.before(nowTimestamp)){
//			return ServiceUtil.returnError(UtilProperties.getMessage("HrCommonUiLabels", "HolidayIsbeforeNow", locale));
//		}
//		List<GenericValue> holiday= delegator.findList("Holidays", EntityCondition.makeCondition("holidayTypeIds",holidayTypeIds),null, null, null, false);
		
//		if(UtilValidate.isNotEmpty(holiday)){
//			for(GenericValue entry:holiday){
//				
//			}
//			
//		}
		GenericValue holidays = delegator.makeValidValue("Holidays", context);
		holidays.set("fromDate", fromDate);
		holidays.set("thruDate", thruDate);
		holidays.set("holidayId", delegator.getNextSeqId("holidayId"));
		try {
			holidays.create();
		} catch (GenericEntityException e) {
			// TODO Auto-generated catch block
		...
		retMap.put("holidayId", holidayId);		
		return retMap;
	}
	...
	public static Map<String,Object> JQgetListWorkingShift (DispatchContext dpct,Map<String,Object> context){
		...
		try {
			Timestamp nowtimestamp = UtilDateTime.nowTimestamp();
			listAllConditions.add(EntityCondition.makeCondition(EntityUtil.getFilterByDateExpr(nowtimestamp)));
			listAllConditions.add(EntityCondition.makeCondition("workingShiftId",EntityJoinOperator.NOT_EQUAL,"_NA_"));
			listIterator = delegator.find("WorkingShift", EntityCondition.makeCondition(listAllConditions,EntityJoinOperator.AND), null, null, listSortFields, opts);
			if(UtilValidate.isNotEmpty(listIterator)){
				result.put("listIterator", listIterator);
			}
		} catch (Exception e) {
	...
	public static Map<String,Object> JQgetListHolidayInYear (DispatchContext dpct,Map<String,Object> context){
		...
		try {
			...
			listAllConditions.add(EntityCondition.makeCondition(EntityCondition.makeCondition("fromDate", EntityOperator.GREATER_THAN_EQUAL_TO, fromDate)));
			listAllConditions.add(EntityCondition.makeCondition("thruDate", EntityOperator.LESS_THAN_EQUAL_TO, thruDate));
			listIterator = delegator.find("Holidays", EntityCondition.makeCondition(listAllConditions,EntityJoinOperator.AND), null, null, listSortFields, opts);
			if(UtilValidate.isNotEmpty(listIterator)){
				result.put("listIterator", listIterator);
			}
		} catch (Exception e) {
