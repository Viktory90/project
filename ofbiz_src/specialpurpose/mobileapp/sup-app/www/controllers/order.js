...
olbius.controller('CreateOrderController', function($rootScope, $scope, $controller, $timeout, $location,$compile,$cordovaNetwork, OrderService, ProductService, SqlService, GPS) {
	...
	$scope.network = {
		...
		$scope.$on('$cordovaNetwork:online', function(event, networkState){
			if(networkState !== 'unknown' || networkState !== 'none'){
				...
				$scope.network.status = true;
			};
			console.log('networkState' +' '+ networkState);
			console.log('$rootScope.network.Status'+' '+$scope.network.status);
		});	
		$scope.$on('$cordovaNetwork:offline', function(event, networkState){
			if(networkState === 'unknown' || networkState === 'none'){
				$scope.network.type = networkState;
				$scope.network.status = false;
			};
			console.log('networkState' +' '+ networkState);
			console.log('$rootScope.network.Status'+' '+$scope.network.status);
		});
	...
	$scope.submitOrder = function() {
		...
		if($scope.network.status){
			 OrderService.submitOrder(customer, products, $rootScope.showLoading ,$timeout).then(function(data) {
				 ...
				 var infoProductPromo = [];
				 	...
				 	if(infoProductPromo && infoProductPromo.length > 0){
				 		for(var pp in infoProductPromo){
								 ...
								 renderTable +="<td>" + infoProductPromo[pp].productPrice + "</td>";
				 		}
				 	};
				 	renderTable += "</tr>";
				 ...
				 $timeout(function(){
					 	console.log('create order Offline');
			...
			var point = GPS.getLocationCallback().then(function(pos){
				...
				if(inputTmp && inputTmp.length > 0){
					SqlService.insert("orderinfo",fields,inputTmp).then(function(res){
						log("insert orderinfo sucess" +JSON.stringify(res));
	...
	$scope.insertOrderOffline = function(input,id){
		console.log('vao insert offline');
		if(id){
			for(var x in input){
				input[x].push(id);
			}	
		...
		SqlService.insert("orders", fields, input).then(function(res) {
			console.log("create order success" + JSON.stringify(res));
			$scope.showNotify("Tạo đơn Offline thành công", true);
			$scope.initQuantity($scope.products);
			$("#submitOrderBt").hide();
		});
	};
	...
	$scope.calculatePromotions = function(data, product) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if (cond && action) {
				console.log("cond + action " + JSON.stringify(cond) + " " + JSON.stringify(action));
				if (!action.amount) {
		...
		console.log('result calculate Promotions' + JSON.stringify($scope.promotions));
	};
	...
	$scope.$watch('promosProcessed', function() {
		...
		if ($scope.promosProcessed && length && $scope.promosProcessed >= length) {
			if ($scope.promotions.length) {
				var str = "<table class='table table-striped table-bordered table-hover products'>" + "<thead><tr><th class='center'>Mã SP</th>" + "<th class='center'>Tên SP</th><th class='center'>SL</th><th class='center'>Giá</th></tr></thead><tbody>";
				for (var x in $scope.promotions) {
					var obj = $scope.promotions[x];
					str += $scope.renderProductPromotions(obj);
				}
				str += "</tbody></table>";
				$("#promotions").html(str);
			}
	...
	$scope.processOrderPromotion = function(data, totalPayment) {
		...
		for (var x in data) {
			...
			console.log("current cond value" + tmp);
		...
		console.log("after calculated order amount" + total);
		return total;
	};
