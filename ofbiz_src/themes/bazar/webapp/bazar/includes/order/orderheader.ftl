...
<div class="screenlet">
		...
		<tr>
			<td>
			    <#if orderHeader?has_content>
			      <div class="divinline" style="float:left; padding-right:10px; background:#FFF; padding-left:50px">${uiLabelMap.CommonNbr} </div>
			      <a href="<@ofbizUrl fullPath="true">orderstatus?orderId=${orderHeader.orderId}</@ofbizUrl>" class="button">${orderHeader.orderId}</a>
			    </#if>
		    </td>
	    </tr>
    </table>   
   ...
   <h2>
    ...
    <#if (orderHeader.orderId)?exists>
      ...
      [ <a class="linkcolor" href="<@ofbizUrl fullPath="true">order.pdf?orderId=${(orderHeader.orderId)?if_exists}</@ofbizUrl>" class="lightbuttontext">PDF</a> ]
    </#if>
  </h2>
