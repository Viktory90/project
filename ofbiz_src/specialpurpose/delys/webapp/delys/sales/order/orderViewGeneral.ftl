				...
				<div class="form-horizontal basic-custom-form" style="display: block;">
					<table cellspacing="0" cellpadding="1" border="0" class="table table-bordered">
						<thead>
							...
							<tr style="font-weight: bold;">
								<td colspan="2">${uiLabelMap.DAProductId} - ${uiLabelMap.DAProductName}</td>
		                        ...
		                        <#if productId?exists && productId == "shoppingcart.CommentLine">
		              			...
		              			<#else>
									...
									<td class="color-red align-right">
										${itemLine.invoicePrice?if_exists}
									</td>
									<td class="color-red align-right">
										${itemLine.invoiceSubTotal?if_exists}
									</td>
									</#if>
		          				</#if>
							</tr>
				...
				<#if isChiefAccountant || isDistributor>
					...
					<#--<#else>-->
						...
						<#if currentStatus.statusId == "ORDER_SADAPPROVED" || hasPerPaymentOrder>
							<#if hasDistributorApproved == "TRUE" || hasPerPaymentOrder>
											...
											<div class="span6">
												...
												<label>
											 	...
											 	<button id="btn_attachPaymentOrder" type="submit" class="btn btn-small btn-primary">
										 			<i class="fa-floppy-o"></i>&nbsp;&nbsp;${uiLabelMap.CommonSave}</button>
											</div>
										</div>
									</div>
								...
								<div class="row-fluid wizard-actions">
									<#if hasDistributorApproved == "TRUE">
										<form action="<@ofbizUrl>changeOrderStatus</@ofbizUrl>" method="post" style="display:inline-block">
											...
											<input type="hidden" name="ntfId" value="${parameters.ntfId?if_exists}">
											<button class="btn btn-primary" type="submit"><i class="icon-remove open-sans"></i>${uiLabelMap.DACancelOrder}</button>
										</form>
										<form name="confirmPaymentForm" id="confirmPaymentForm" action="<@ofbizUrl>changeOrderStatus</@ofbizUrl>" method="post" style="display:inline-block">
											...
											<input type="hidden" name="changeReason" value="" />
											<input type="hidden" name="ntfId" value="${parameters.ntfId?if_exists}">
											<button class="btn btn-primary" type="button" onClick="sendConfirmPayment();">${uiLabelMap.DASendConfirm} <i class="icon-angle-right icon-on-right"></i></button>
										</form>
									</#if>
								</div>
							</#if>
...
<script type="text/javascript">
	function sendConfirmPayment() {
		...
		if (!isSend) {
			bootbox.dialog("${uiLabelMap.DAYouNotYetAttachPaymentOrder}!", [{
				"label" : "OK",
