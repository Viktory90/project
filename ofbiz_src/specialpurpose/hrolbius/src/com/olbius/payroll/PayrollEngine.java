...
public class PayrollEngine {
	...
	public static void getRefPayrollParameter(List<EntityParameter> parameters, GenericValue userLogin,
		...
		if(code.equals("LUONG_CO_BAN")){
			...
			for(GenericValue tempGv: partyInsuranceSalary){
				salaryFromDate = tempGv.getTimestamp("fromDate");
				salaryThruDate = tempGv.getTimestamp("thruDate");
				...
				String value = "0";
				BigDecimal salaryInsurance = tempGv.getBigDecimal("salaryInsurance");
				...
				//FIXME don't use hard fix "MONTHLY" periodType 
			...
			List<GenericValue> partyParicipateInsurance = delegator.findList("PartyParticipateInsurance", EntityCondition.makeCondition(EntityCondition.makeCondition("partyId", partyId),
																																		EntityOperator.AND,
																																		EntityCondition.makeCondition("insuranceTypeId", code)), null, UtilMisc.toList("-statusDatetime"), null, false);
			String value = "0";
...
//			conn.close();
	...
	public static Map<String, Object> getPayrolls(DispatchContext ctx,
		...
		try {
			listIterator = delegator.find("PayrollTableRecord", tmpCond, null, null,
					listSortFields, opts);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	@SuppressWarnings("unchecked")
	public static Map<String, Object> getPartyFormulaInvoiceItemType(DispatchContext ctx,
			Map<String, Object> context) {
		Delegator delegator = ctx.getDelegator();
		Map<String, Object> successResult = ServiceUtil.returnSuccess();
		EntityListIterator listIterator = null;
	...
	public static Map<String, Object> getPayrollParameters(DispatchContext ctx,
		...
		EntityCondition tmpCond = EntityCondition.makeCondition(
				listAllConditions, EntityOperator.AND);
		try {
			listIterator = delegator.find("PayrollParameters", tmpCond, null, null,
					listSortFields, opts);
		} catch (Exception e) {
			e.printStackTrace();
		}
