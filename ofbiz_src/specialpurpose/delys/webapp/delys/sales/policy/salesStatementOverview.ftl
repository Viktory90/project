...
<#if salesStatementSelected?exists>
	...
	<div class="widget-header widget-header-blue widget-header-flat">
		<h4 class="lighter">${uiLabelMap.DAViewSalesStatement}: ${salesStatementSelected.salesName?if_exists} 
			<#if salesStatementSelected.salesId?exists>
				(<a href="<@ofbizUrl>viewSalesStatement?salesId=${salesStatementSelected.salesId}</@ofbizUrl>">${salesStatementSelected.salesId}</a>)
			</h4>
			<span class="widget-toolbar none-content">
				<#if currentStatusId?exists && currentStatusId == "SALES_SM_CREATED">
					<a href="<@ofbizUrl>editSalesStatement?salesId=${salesStatementSelected.salesId?if_exists}</@ofbizUrl>">
						<i class="icon-pencil open-sans">${uiLabelMap.DAEditSalesStatement}</i>
					</a>
				</#if>
				<a href="<@ofbizUrl>newSalesStatement</@ofbizUrl>">
					<i class="icon-plus open-sans">${uiLabelMap.DACreateNewSalesStatement}</i>
				</a>
			</span>
		<#else>
			</h4>
