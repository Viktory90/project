...
<fo:block font-size="11" font-family="Times New Roman">
	 <fo:table table-layout="fixed" space-after.optimum="10pt">
    	...
    	<fo:table-body>
	    	<fo:table-row>
		        <fo:table-cell width="50px" >
		        	<fo:block font-weight="bold">Đơn vị:</fo:block>
		        	<fo:block font-weight="bold">Địa chỉ:</fo:block>
		        </fo:table-cell>
		        <fo:table-cell font-size="11">
		        	<fo:block>${partyFrom?if_exists.firstName?if_exists} ${partyFrom?if_exists.middleName?if_exists} ${partyFrom?if_exists.lastName?if_exists} ${partyFrom?if_exists.groupName?if_exists}</fo:block>
		        	<fo:block>${addressFrom?if_exists.address1?if_exists} ${addressFrom?if_exists.address2?if_exists} </fo:block>
		        </fo:table-cell>
		        <fo:table-cell width="150px">
		        	<fo:block font-weight="bold">Mẫu số: 02 - VT</fo:block>
		        	<fo:block>Ban hành theo QĐ số 1141-TC/QĐ/CĐKT</fo:block>
		        	<fo:block>Ngày 01 tháng 11 năm 1995 của bộ tài chính</fo:block>
		        </fo:table-cell>
	      	</fo:table-row>
	    </fo:table-body>
	 </fo:table>
	    ...
	    <fo:table-body>
		    <fo:table-row >
			   ...
			   <fo:table-cell >
			       ...
			       <fo:block>${deliveryType?if_exists.description?if_exists} <fo:leader /> </fo:block>
