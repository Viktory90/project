 ...
 */
olbius.controller('CustomerDetailController', function($rootScope, $scope, $controller, $routeParams, $location, CustomerService) {
	...
	$scope.$on('$viewContentLoaded', function() {
		$scope.setHeader("Chi tiết khách hàng", "/customer", false);
		$scope.initCustomer();
	...
	$scope.updateLocationCustomer = function(){
		var point = $scope.getMarkerPoint();
			CustomerService.updateLocationCustomer($routeParams.id,point.latitude,point.longitude).then(function(data){
				if(data){
					bootbox.alert("Cập nhật thành công,đang chờ duyệt");
	...
	if($scope.groupName && $scope.groupName.length){	
		...
		bootbox.alert("Bạn chưa thay đổi nội dung,hãy nhập lại");
	}
	};
	$scope.back = function() {
