package com.olbius.delys.order;
...
//		Security security = ctx.getSecurity();
		...
		try {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceDelys_error, "DAErrorRequirementTypeLookupFailed", locale) + e.toString());
		}
		
		// make sure we have a valid requirement type
		if (requirementType == null) {
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceDelys_error, 
		...
		if (deliveryReqItems.size() < 1) {
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceDelys_error, "DAItemsNone", locale));
		}
			...
			if (currentOrderId != null) {
				try {
					...
					if (UtilValidate.isEmpty(currentOrder)) {
						String errMsg = UtilProperties.getMessage(resourceDelys_error, "DAProductNotFound", new Object[] { currentOrderId }, locale);
	    ...
	    try {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resourceDelys_error, "DADeliveryRequirementCreationFailedPleaseNotifyCustomerService", locale));
		}
...
//			}
    ...
    private static boolean hasPermission(String orderTypeId, String partyId, GenericValue userLogin, String action, Security security, String notCreateInventoryItem) {
        boolean hasPermission = security.hasEntityPermission("ORDERMGR", "_" + action, userLogin);
        if (!hasPermission && orderTypeId.equals("SALES_ORDER")) hasPermission = security.hasEntityPermission("ORDERSAL", "_DIS_" + action, userLogin);
        if (!hasPermission && (orderTypeId.equals("PURCHASE_ORDER") || "Y".equals(notCreateInventoryItem))) hasPermission = security.hasEntityPermission("ORDERPUR", "_DIS_" + action, userLogin);
        if (!hasPermission) {
            if (orderTypeId.equals("SALES_ORDER")) {
                if (security.hasEntityPermission("ORDERMGR", "_SALES_" + action, userLogin)) {
                    hasPermission = true;
...
//                            " and for quantity: " + techDataCalendarExcDay.getDouble("usedCapacity").toString(), module);
       ...
       if (UtilValidate.isNotEmpty(workEffortId)) {
        ...
        if (UtilValidate.isNotEmpty(orderItemAssociations)) {
            for (GenericValue orderItemAssociation : orderItemAssociations) {
                ...
                if (orderItemFrom != null && orderHeaderFrom != null) {
                	com.olbius.delys.order.OrderReadHelper orderOrderReadHelper = new com.olbius.delys.order.OrderReadHelper(orderHeaderFrom);
        ...
        // TODOCHANGE: create notify to SUP
    	...
    	try {
    		...
    		if (listPlacingCustomerGe != null) {
 				...
 				if (listPlacingCustomer != null) {
 					for (String placingCustomer : listPlacingCustomer) {
 						List<String> listPartyTmp = SalesPartyUtil.getSUPsByDistributor(delegator, placingCustomer);
    		...
    		/*if (UtilValidate.isNotEmpty(partiesRolePlacing)) {
    			for (String partyRolePlacing : partiesRolePlacing) {
    				List<String> listPartyTmp = SalesPartyUtil.getSUPsByDistributor(delegator, partyRolePlacing);
        			if (listPartyTmp != null) {
        				partiesList.addAll(listPartyTmp);
        			}
        		}
    		}*/
    		header = UtilProperties.getMessage(resourceDelys, "DAApproveOrder",locale) + " [" + orderId +"]";
    		action = "orderView";
    ...
    public static Map<String, Object> checkOrderItemAssocStatus(DispatchContext ctx, Map<String, ? extends Object> context) {
        ...
        if (orderItem == null) {
        	...
        	return ServiceUtil.returnError(UtilProperties.getMessage(resourceDelys_error, "DACannotUpdateNullOrderHeader",UtilMisc.toMap("orderId",orderId),locale));
        }
         	...
         	try {
         		...
         		if (orderHeader != null) {
         			...
         			if ("ORDER_SUPAPPROVED".equals(statusId)) {
             			...
             			header = UtilProperties.getMessage(resourceDelys, "DAApproveOrderAndAttachPaymentOrder",locale) + " [" + orderId +"]";
                    	...
                    	List<GenericValue> listPlacingCustomerGe = delegator.findByAnd("OrderRole", UtilMisc.toMap("orderId", orderId, "roleTypeId", "PLACING_CUSTOMER"), null, false);
             			...
             			ntfType = "ONE";
             			header = UtilProperties.getMessage(resourceDelys, "DAOrderComplete",locale) + " [" + orderId +"]";
                    } else if ("ORDER_CANCELLED".equals(statusId)) {
                    	// send to customer (distributor)
                    	List<GenericValue> listPlacingCustomerGe = delegator.findByAnd("OrderRole", UtilMisc.toMap("orderId", orderId, "roleTypeId", "PLACING_CUSTOMER"), null, false);
             			if (listPlacingCustomerGe != null) {
             			...
             			ntfType = "ONE";
             			header = UtilProperties.getMessage(resourceDelys, "DAOrderIsCanceled",locale) + " [" + orderId +"]";
    ...
    // TODOCHANGE createOrder for distributor
 	...
 	public static Map<String, Object> createOrderDis(DispatchContext ctx, Map<String, ? extends Object> context) {
         ...
         //  PURCHASE ORDERS - if there is a PURCHASE_ORDER permission
         Map<String, Object> resultSecurity = new HashMap<String, Object>();
         boolean hasPermission = OrderServices.hasPermission(orderTypeId, partyId, userLogin, "CREATE", security, notCreateInventoryItem);
         // final check - will pass if userLogin's partyId = partyId for order or if userLogin has ORDERMGR_CREATE permission
         // jacopoc: what is the meaning of this code block? FIXME
         if (!hasPermission) {
             partyId = ServiceUtil.getPartyIdCheckSecurity(userLogin, security, context, resultSecurity, "ORDERMGR", "_CREATE");
...
//                             " and for quantity: " + techDataCalendarExcDay.getDouble("usedCapacity").toString(), module);
        ...
        if (UtilValidate.isNotEmpty(workEffortId)) {
         ...
         try {
             ...
             // START inventory reservation
             // TODOCHANGE createOrder don't create inventory item
     ...
     public static Map<String, Object> addItemToApprovedOrderSales(DispatchContext dctx, Map<String, ? extends Object> context) {
         ...
         Locale locale = (Locale) context.get("locale");
 		 try {
 			 Map<String, Object> result = dispatcher.runSync("appendOrderItem", context);
	 		 result.remove("shoppingCart");  //remove extra parameter
	         return result;
	     } catch (GenericServiceException e) {
	         Debug.logError (e, "Add item into order items: " + e.toString(), module);
	         return ServiceUtil.returnError(UtilProperties.getMessage(resourceDelys, "DAErrorAddItemToOrder",
