...
(function() {
		...
		for (var y in data) {
			...
			if(!currentPr || currentPr != obj.productId){
				...
				(function(){
					...
					setTimeout(function(){
						addProductType(id, $("#"+id+obj.productId + len + " button"), {
							quantity: obj.quantity,
							uomId: obj.uomId
						});	
					}, 50);
	...
	function addAddress(id, value) {
		...
		if (addr.length) {
			...
			prspl.find("button").click(function(){
				addProductType("productSampling", $(this));
			});
			var prsl = $("#productSelling"+id+len1); 
			prsl.find("button").click(function(){
				addProductType("productSelling", $(this));
			});
	...
	function addProductType(type, obj, value){
		var uom = value && value.uomId ? value.uomId : "";
