...
import com.olbius.delys.order.OrderReadHelper;
import com.olbius.delys.order.ShoppingCartEvents;
import com.olbius.delys.order.ShoppingCartHelper;
	...
	public static Map<String, Object> updateProductPlanItem(DispatchContext ctx, Map<String, Object> context) throws GenericEntityException {
		...
		if ("PURCHASE_ORDER".equals((String)orderHeader.get("orderTypeId"))){
			...
			if (!listPlanByOrders.isEmpty()){
				...
				List<GenericValue> orderItems = delegator.findList("OrderItem", EntityCondition.makeCondition(UtilMisc.toMap("orderId", orderId)), null, null, null, false);
				if (!orderItems.isEmpty() && !planItems.isEmpty()){
					if("ORDER_COMPLETED".equals(currentOrderStatus)){
						for (GenericValue planItem : planItems){
							if (planItem.getBigDecimal("recentPlanQuantity") == null){
								planItem.put("recentPlanQuantity", new BigDecimal(0));
							} 
							for (GenericValue orderItem : orderItems){
								if (orderItem.get("productId").equals(planItem.get("productId"))){
									BigDecimal quantity = orderItem.getBigDecimal("quantity");
									planItem.put("recentPlanQuantity", planItem.getBigDecimal("recentPlanQuantity").add(quantity));
								}
							}
							delegator.store(planItem);
							if (planItem.getBigDecimal("recentPlanQuantity").compareTo(planItem.getBigDecimal("planQuantity")) >= 0){
								planItem.put("statusId", "PLAN_ITEM_COMPLETED");
								delegator.store(planItem);
							}
						}
					} else {
						if("ORDER_CANCELLED".equals(currentOrderStatus)){
							for (GenericValue planItem : planItems){
								for (GenericValue orderItem : orderItems){
									if (orderItem.get("productId").equals(planItem.get("productId"))){
										BigDecimal quantity = orderItem.getBigDecimal("quantity");
										planItem.put("orderedQuantity", planItem.getBigDecimal("orderedQuantity").subtract(quantity));
									}
								}
								delegator.store(planItem);
							}
							...
							if ("ORDER_APPROVED".equals(currentOrderStatus)){
								for (GenericValue planItem : planItems){
									for (GenericValue orderItem : orderItems){
										if (orderItem.get("productId").equals(planItem.get("productId"))){
											BigDecimal quantity = orderItem.getBigDecimal("quantity");
											planItem.put("orderedQuantity", planItem.getBigDecimal("orderedQuantity").add(quantity));
										}
									}
									delegator.store(planItem);
								}
...
//			currencyUomId = (String)productPlan.get("currencyUomId");
		...
		try {
			...
			if (weekETD == null){
			    ...
			    GenericValue customTimeETD = EntityUtil.getFirst(delegator.findList("CustomTimePeriod", EntityCondition.makeCondition(UtilMisc.toMap("fromDate", fromDateETD)), null, null, null, false));
		...
		if (agreementId != null){
			try {
				...
				try {
					...
					fromAddress = EntityUtil.filterByDate(fromAddress);
					if (fromAddress.isEmpty()){
						List<GenericValue> fromOldAddress = delegator.findList("AgreementPartyCTMPurpose", EntityCondition.makeCondition(UtilMisc.toMap("agreementId", agreementId, "partyId", partyIdFrom, "contactMechTypeId", "EMAIL_ADDRESS")), null, null, null, false);
						if (!fromOldAddress.isEmpty()){
							for (GenericValue add : fromOldAddress){
								add.put("thruDate", UtilDateTime.nowTimestamp());
								delegator.store(add);
				...
				if (finAccountIdFroms != null && !finAccountIdFroms.isEmpty()){
					for (String finAccId : finAccountIdFroms){
						GenericValue finAccTermFrom = delegator.makeValue("AgreementTerm");
						finAccTermFrom.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
						finAccTermFrom.put("termTypeId", "FIN_ACC_TERM");
						finAccTermFrom.put("agreementId", agreementId);
						finAccTermFrom.put("textValue", finAccId);
						finAccTermFrom.put("fromDate", UtilDateTime.nowTimestamp());
						try {
							delegator.createOrStore(finAccTermFrom);
						} catch (GenericEntityException e) {
							e.printStackTrace();
						}
					}
				...
				if (finAccountIdTos != null && !finAccountIdTos.isEmpty()){
					for (String finAccId : finAccountIdTos){
						GenericValue finAccTermTo = delegator.makeValue("AgreementTerm");
						finAccTermTo.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
						finAccTermTo.put("termTypeId", "FIN_ACC_TERM");
						finAccTermTo.put("agreementId", agreementId);
						finAccTermTo.put("textValue", finAccId);
						finAccTermTo.put("fromDate", UtilDateTime.nowTimestamp());
						try {
							delegator.createOrStore(finAccTermTo);
						} catch (GenericEntityException e) {
							e.printStackTrace();
						}
					}
		...
		if (portOfDischargeId != null){
			Boolean exist = false;
			try {
				List<GenericValue> portOfDischarges = delegator.findList("AgreementTerm", EntityCondition.makeCondition(UtilMisc.toMap("agreementId", agreementId, "termTypeId", "PORT_OF_CHARGE")), null, null, null, false);
				portOfDischarges = EntityUtil.filterByDate(portOfDischarges);
				if (!portOfDischarges.isEmpty()){
					for (GenericValue port : portOfDischarges){
						if (!portOfDischargeId.equals((String)port.get("textValue"))){
							port.put("thruDate", UtilDateTime.nowTimestamp());
							delegator.store(port);
						} else {
							exist = true;
						}
					}
				...
				if (!exist){
					GenericValue finAccTermTo = delegator.makeValue("AgreementTerm");
					finAccTermTo.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
					finAccTermTo.put("termTypeId", "PORT_OF_CHARGE");
					finAccTermTo.put("agreementId", agreementId);
					finAccTermTo.put("textValue", portOfDischargeId);
					finAccTermTo.put("fromDate", UtilDateTime.nowTimestamp());
					delegator.createOrStore(finAccTermTo);
				}
			} catch (GenericEntityException e) {
		...
		if (weekETA != null){
			Boolean exist = false;
			try {
				List<GenericValue> weekETAs = delegator.findList("AgreementTerm", EntityCondition.makeCondition(UtilMisc.toMap("agreementId", agreementId, "termTypeId", "ETA_AGREEMENT_TERM")), null, null, null, false);
				weekETAs = EntityUtil.filterByDate(weekETAs);
				if (!weekETAs.isEmpty()){
					for (GenericValue term : weekETAs){
						if (!weekETA.equals((String)term.get("textValue"))){
				...
				if (!exist){
					GenericValue dateTimeTerm = delegator.makeValue("AgreementTerm");
					dateTimeTerm.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
					dateTimeTerm.put("termTypeId", "ETA_AGREEMENT_TERM");
					dateTimeTerm.put("agreementId", agreementId);
					dateTimeTerm.put("textValue", weekETA);
					dateTimeTerm.put("fromDate", UtilDateTime.nowTimestamp());
					delegator.createOrStore(dateTimeTerm);
				}
			} catch (GenericEntityException e) {
				e.printStackTrace();
			}
		...
		if (weekETD != null){
			Boolean exist = false;
			try {
				List<GenericValue> weekETDs = delegator.findList("AgreementTerm", EntityCondition.makeCondition(UtilMisc.toMap("agreementId", agreementId, "termTypeId", "ETD_AGREEMENT_TERM")), null, null, null, false);
				weekETDs = EntityUtil.filterByDate(weekETDs);
				if (!weekETDs.isEmpty()){
					for (GenericValue term : weekETDs){
						if (!weekETA.equals((String)term.get("textValue"))){
							term.put("thruDate", UtilDateTime.nowTimestamp());
							delegator.store(term);
						} else {
				...
				if (!exist){
					GenericValue dateTimeTerm = delegator.makeValue("AgreementTerm");
					dateTimeTerm.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
					dateTimeTerm.put("termTypeId", "ETD_AGREEMENT_TERM");
					dateTimeTerm.put("agreementId", agreementId);
					dateTimeTerm.put("textValue", weekETD);
					dateTimeTerm.put("fromDate", UtilDateTime.nowTimestamp());
					delegator.createOrStore(dateTimeTerm);
				}
			} catch (GenericEntityException e) {
		...
		if (currencyUomId != null){
			GenericValue currencyTerm = delegator.makeValue("AgreementTerm");
			currencyTerm.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
			currencyTerm.put("termTypeId", "DEFAULT_PAY_CURRENCY");
			currencyTerm.put("agreementId", agreementId);
			currencyTerm.put("textValue", currencyUomId);
			currencyTerm.put("fromDate", UtilDateTime.nowTimestamp());
			try {
				delegator.createOrStore(currencyTerm);
			} catch (GenericEntityException e) {
				e.printStackTrace();
			}
			if (!currencyUomIds.isEmpty()){
				for (String uomId : currencyUomIds){
					if (!uomId.equals(currencyUomId)){
						GenericValue currencyTermOther = delegator.makeValue("AgreementTerm");
						currencyTermOther.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
						currencyTermOther.put("termTypeId", "OTHER_PAY_CURRENCY");
						currencyTermOther.put("agreementId", agreementId);
						currencyTermOther.put("textValue", uomId);
						currencyTermOther.put("fromDate", UtilDateTime.nowTimestamp());
						try {
							delegator.createOrStore(currencyTermOther);
						} catch (GenericEntityException e) {
							e.printStackTrace();
						}
					}
			...
			if (!currencyUomIds.isEmpty()){
				...
				currencyTerm.put("termTypeId", "DEFAULT_PAY_CURRENCY");
				currencyTerm.put("agreementId", agreementId);
				currencyTerm.put("textValue", currencyUomIds.get(0));
				currencyTerm.put("fromDate", UtilDateTime.nowTimestamp());
				try {
					delegator.createOrStore(currencyTerm);
				} catch (GenericEntityException e) {
		...
		GenericValue transshipmentTerm = delegator.makeValue("AgreementTerm");
		transshipmentTerm.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
		transshipmentTerm.put("termTypeId", "TRANS_SHIPMENT");
		transshipmentTerm.put("agreementId", agreementId);
		transshipmentTerm.put("fromDate", UtilDateTime.nowTimestamp());
		if ("Y".equals(transshipment)){
			transshipmentTerm.put("textValue", "Y");
		} else {
			transshipmentTerm.put("textValue", "N");
		}
		GenericValue partialShipmentTerm = delegator.makeValue("AgreementTerm");
		partialShipmentTerm.put("agreementTermId", delegator.getNextSeqId("AgreementTerm"));
		partialShipmentTerm.put("termTypeId", "PARTIAL_SHIPMENT");
		partialShipmentTerm.put("agreementId", agreementId);
		partialShipmentTerm.put("fromDate", UtilDateTime.nowTimestamp());
		if ("Y".equals(partialShipment)){
			partialShipmentTerm.put("textValue", "Y");
		} else {
			partialShipmentTerm.put("textValue", "N");
		}
		try {
			delegator.createOrStore(partialShipmentTerm);
			delegator.createOrStore(transshipmentTerm);
		} catch (GenericEntityException e) {
			e.printStackTrace();
		}
		try {
			...
			if(!productPlanItemByLots.isEmpty()){
	    		for (GenericValue item : productPlanItemByLots){
	    			BigDecimal quantity = item.getBigDecimal("lotQuantity");
	    			...
	    			helper.addToCart(catalogId, null, null, productId, productCategoryId, "PRODUCT_ORDER_ITEM", null, null, null, quantity, null, null, null, null, null, shipBeforeDate, shipAfterDate, null, null, context, null);
	    		}
	    		CheckOutHelper checkOutHelper = new CheckOutHelper(dispatcher, delegator, cart);
    	        Map<String, Object> callResult;
    	        ...
    	        boolean areOrderItemsExploded = CheckOutEvents.explodeOrderItems(delegator, cart);
    	        callResult = checkOutHelper.createOrder(userLogin, null, null, null, areOrderItemsExploded, null, null);
                String orderId = (String)callResult.get("orderId");
    	        if (productPlanId != null){
    	        	try {
//    	        		thruDate chua co, doan sau viet lai 1 service rieng dde thuc hien viec update bang ProductPlanAndOrder by datnv
    					GenericValue planAndOrder = delegator.makeValue("ProductPlanAndOrder");
    					planAndOrder.put("orderId", orderId);
    					planAndOrder.put("productPlanId", productPlanId);
    					planAndOrder.put("fromDate", UtilDateTime.nowTimestamp());
//    					planAndOrder.put("thruDate", thruDate);
    	        ...
    	        try {
					GenericValue agreementAndOrder = delegator.makeValue("AgreementAndOrder");
					agreementAndOrder.put("orderId", orderId);
					agreementAndOrder.put("agreementId", agreementId);
					agreementAndOrder.put("fromDate", UtilDateTime.nowTimestamp());
					delegator.createOrStore(agreementAndOrder);
				} catch (GenericEntityException e) {
					e.printStackTrace();
				}
    	        for (GenericValue item : productPlanItemByLots){
...
//END HISTORY INVENTORY
			...
			try {
				...
				if(!listLogSpecialist.isEmpty()){
					for (GenericValue managerParty : listLogSpecialist){
						String tagetLink = "requirementId="+requirementId+";orderId="+orderId;
	...
	public static Map<String, Object> updateReceiptRequirement(DispatchContext ctx, Map<String, Object> context){
		Map<String, Object> result = new FastMap<String, Object>();
		Delegator delegator = ctx.getDelegator();
		String requirementId = (String)context.get("requirementId");
		String facilityId = (String)context.get("facilityId");
		...
		notifyParam.put("header", "Co yeu cau nhap hang moi");
        try {
	...
	public static Map<String,Object> updateOrderItemFromDocuments(DispatchContext ctx, Map<String,Object> context){
		...
		try {
			...
			if (orderItem != null){
				...
				orderItem.put("datetimeManufactured", datetimeManufactured);
				delegator.store(orderItem);
			}
			else{
				List<GenericValue> listShipGroupSeqId = delegator.findList("OrderItemShipGroup", EntityCondition.makeCondition(UtilMisc.toMap("orderId",orderId)), null, null, null, false);
				GenericValue shipGroupId = EntityUtil.getFirst(listShipGroupSeqId);
				if(shipGroupId != null){
			...
			List<GenericValue> listContainerByOrder = delegator.findList("OrderAndContainer", EntityCondition.makeCondition(UtilMisc.toMap("orderId", orderId)), null, null, null, false);
			if (!listContainerByOrder.isEmpty()){
				String containerId = (String)listContainerByOrder.get(0).get("containerId");
				if (containerId != null){
					if(newSeqId == true){
						GenericValue newContainerItem = delegator.makeValue("ContainerItem");
						newContainerItem.put("containerId", containerId);
						delegator.setNextSubSeqId(newContainerItem, "containerItemSeqId", 5, 1);
						newContainerItem.put("quantity", documentQuantity);
						newContainerItem.put("expireDate", expireDate);
						newContainerItem.put("fromOrderId", orderId);
						newContainerItem.put("fromOrderItemSeqId", orderItemSeqId);
						newContainerItem.put("productId", productId);
						delegator.create(newContainerItem);
					}else{
						List<GenericValue> containerItems = delegator.findList("ContainerItem", EntityCondition.makeCondition(UtilMisc.toMap("containerId", containerId, "fromOrderId", orderId, "fromOrderItemSeqId", orderItemSeqId)), null, null, null, false);
						GenericValue containerItem = containerItems.get(0);
						containerItem.put("quantity", documentQuantity);
						containerItem.put("productId", productId);
						containerItem.put("expireDate", expireDate);
						delegator.store(containerItem);
					}
				}
			}
			List<GenericValue> listReceiptByOrder = delegator.findList("Receipt", EntityCondition.makeCondition(UtilMisc.toMap("orderId", orderId)), null, null, null, false);
			if (!listReceiptByOrder.isEmpty()){
				String receiptId = (String)listReceiptByOrder.get(0).get("receiptId");
				if (receiptId != null){
					if(newSeqId == true){
						GenericValue newReceiptItem = delegator.makeValue("ReceiptItem");
						newReceiptItem.put("receiptId", receiptId);
						delegator.setNextSubSeqId(newReceiptItem, "receiptItemSeqId", 5, 1);
						newReceiptItem.put("orderedQuantity", documentQuantity);
						newReceiptItem.put("expireDate", expireDate);
						newReceiptItem.put("fromOrderId", orderId);
						newReceiptItem.put("fromOrderItemSeqId", orderItemSeqId);
						newReceiptItem.put("productId", productId);
						//thieu unitPrice, currencyUomId, quantityUomId... lien quan den Supplier, Product..
						//chỉnh sửa sau khi có chức năng thêm mới sản phẩm và Supplier tương ứng
						delegator.create(newReceiptItem);
					}else{
						List<GenericValue> receiptItems = delegator.findList("ReceiptItem", EntityCondition.makeCondition(UtilMisc.toMap("receiptId", receiptId, "fromOrderId", orderId, "fromOrderItemSeqId", orderItemSeqId)), null, null, null, false);
						if(!UtilValidate.isEmpty(receiptItems)){
							GenericValue receiptItem = receiptItems.get(0);
							receiptItem.put("orderedQuantity", documentQuantity);
							receiptItem.put("expireDate", expireDate);
							delegator.store(receiptItem);
						}
					}
				}
...
//    				String dateStr = agreeDate.toString();
    				...
    				row.put("agreementDate", dateStr2);
...
//		String planId = (String)context.get("planId");
	...
	//create Lot
				...
				try {
					...
					//tim plan tuan
					listPlanTimePeriod = delegator.findList("ProductPlanHeader", EntityCondition.makeCondition(UtilMisc.toMap("customTimePeriodId", customTimePeriodId, "internalPartyId", internalPartyId)), null, null, null, false);
				} catch (GenericEntityException e1) {
					// TODO Auto-generated catch block
					return ServiceUtil.returnError(UtilProperties.getMessage(resource,
				...
				if(created == true){
					...
					String statusId = "PLAN_CREATED";
//					productPlanHeaderGe.put("statusId", statusId);
//					try {
...
//			String yearStr = calendar.get(Calendar.YEAR).toString();
			...
			if(listCustomTimeMonth != null){
				...
				String thisStatus = (String) status.get("description");
...
//End Copy
	...
	public static Map<String,Object> testPopup(DispatchContext ctx, Map<String,Object> context) throws GenericEntityException{
        Delegator delegator = ctx.getDelegator();
		Map<String, Object> result = new FastMap<String, Object>();
		String json = (String)context.get("json");
        ...
        result.put("product", listProduct);
        return result;
	}
...
//			oneCont.add(less);
	...
	public static void updateSttPlanHeader(Delegator delegator, String productPlanId, String task) throws GenericEntityException{
		...
		if(task != null){
			if(task == "CONTCREATED"){
				productPlHeader.put("statusId", "PLAN_CONTCREATED");
...
//						lotPlan.put("statusId", "LOT_CREATED");
	...
	public static Map<String,Object> devideContainer(DispatchContext ctx, Map<String,Object> context) throws GenericEntityException{
		...
		if(!UtilValidate.isEmpty(listProductPlanHeader)){
			...
			String productPlanWeekId = (String)productPlanWeekGe.get("productPlanId");
			String sttPlan = (String)productPlanWeekGe.get("statusId");
			...
			if(sttPlan.equals("PLAN_CONTCREATED") || sttPlan.equals("PLAN_ORDERED") || sttPlan.equals("PLAN_COMPLETED")){
//				result.put("contCreated", "container is created");
				result = getProductPlanAndLot(delegator, productPlanWeekId);
...
//				result.put("contCreated" , "container is not created");
				devidePlanToContainer(delegator, productPlanWeekId);
				result = getProductPlanAndLot(delegator, productPlanWeekId);
				result.put("productPlanHeader", productPlanWeekId);
			}else{
			...
			//DU Lieu Tuan Chuwa duoc luu
			result.put("contCreated", "plan is not created");
