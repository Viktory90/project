...
<div>
    <div class="cartbutton">
          ...
          <#if (shoppingCartSize > 0)>
            <a href="javascript:document.cartform.submit();" class="button">${uiLabelMap.EcommerceRecalculateCart}</a>
            <a href="<@ofbizUrl>emptycart</@ofbizUrl>" class="button">${uiLabelMap.EcommerceEmptyCart}</a>
            <a href="javascript:removeSelected();" class="button">${uiLabelMap.EcommerceRemoveSelected}</a>
      ...
      <table>
        <thead>
            <tr>
              ...
              <#else>
                <td class="quantity" scope="row">${uiLabelMap.CommonQuantity}</td>
              </#if>
              <td class="price" scope="row">${uiLabelMap.EcommerceUnitPrice}</td>
              <td class="price" scope="row">${uiLabelMap.EcommerceAdjustments}</td>
              <td class="total" scope="row">${uiLabelMap.EcommerceItemTotal}</td>
            ...
            <td class="quantity">
                ...
                <#else><#-- Is Promo or Shoppinglist -->
                       <#if fixedAssetExist == true><#if cartLine.getReservStart()?exists><table><tr><td>&nbsp;</td><td><input type="text" class="inputBox" size="10" name="reservStart_${cartLineIndex}" value=${cartLine.getReservStart()?string}/></td><td><input type="text" class="inputBox" size="2" name="reservLength_${cartLineIndex}" value="${cartLine.getReservLength()?string.number}"/></td></tr><tr><td>&nbsp;</td><td><input type="text" class="inputBox" size="3" name="reservPersons_${cartLineIndex}" value=${cartLine.getReservPersons()?string.number} /></td><td><#else>
                        ...
                        <input size="1" class="w30" type="text" name="update_${cartLineIndex}" value="${cartLine.getQuantity()?string.number}" /></td></tr></table>
                    <#else><#-- fixedAssetExist -->
                        <input size="1" class="w30" type="text" name="update_${cartLineIndex}" value="${cartLine.getQuantity()?string.number}" />
                    </#if>
                </#if>
            </td>
    ...
    <div>
        <div>
            <form method="post" action="<@ofbizUrl>additem<#if requestAttributes._CURRENT_VIEW_?has_content>/${requestAttributes._CURRENT_VIEW_}</#if></@ofbizUrl>" name="quickaddform" id="quickaddform">
                ...
                <a href="javascript:void(0)" onclick="document.getElementById('quickaddform').submit();" class="button">${uiLabelMap.OrderAddToCart}</a>
...
<#if (shoppingCartSize?default(0) > 0)>
  ${screens.render("component://bigshop/widget/CartScreens.xml#promoUseDetailsInline")}
</#if>
