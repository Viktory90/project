...
public class ProductServices {
	...
	public static Map<String, Object> addAdditionalViewForProduct(DispatchContext dctx, Map<String, ?> context) throws GenericServiceException {
		...
		if (UtilValidate.isNotEmpty(fileName)) {
			...
			Map<String, Object> dataResourceCtx = UtilMisc.toMap("userLogin", context.get("userLogin"), "_uploadedFile_fileName",
					...
					context.get("uploadedFile"), "folder", tenantId.concat("/catalog/products/").concat(productId).concat("/").concat(productContentTypeId), "public", isPublic);
			...
			if (isPublic.equals("Y")) {
				result = addImageResource(dispatcher, delegator, context, JcrClientProvider.WEB_DAV_URI.concat(JcrClientProvider.WSP_DEFAULT).concat(path), productContentTypeId);
			}
			if (isPublic.equals("N")) {
				result = addImageResource(dispatcher, delegator, context, JcrClientProvider.WEB_DAV_URI.concat(JcrClientProvider.WSP_SECURITY).concat(path), productContentTypeId);
			}
	...
	public static Map<String, Object> addImageOverrideFields(DispatchContext dctx, Map<String, ? extends Object> context)
		...
		if (product != null) {
			Map<String, Object> dataResourceCtx = UtilMisc.toMap("userLogin", context.get("userLogin"), "_uploadedFile_fileName", fileName,
					...
					tenantId.concat("/catalog/products/").concat(productId), "public", isPublic);
			...
			String path = (String) result.get("path");
			if (isPublic.equals("Y")) {
				product.put(map.get(fileType), JcrClientProvider.WEB_DAV_URI.concat(JcrClientProvider.WSP_DEFAULT).concat(path));
			} else {
				if (isPublic.equals("N")) {
					product.put(map.get(fileType), JcrClientProvider.WEB_DAV_URI.concat(JcrClientProvider.WSP_SECURITY).concat(path));
				}
			}
			if (fileType.equals("original")) {
				dataResourceCtx = UtilMisc.toMap("userLogin", context.get("userLogin"), "curPath", path, "public", context.get("isPublic"));
				try {
					result = dispatcher.runSync("jackrabbitScaleImageService", dataResourceCtx);
				} catch (GenericServiceException e) {
					throw new GenericServiceException(e);
	...
	public static Map<String, Object> addImageForProductPromo(DispatchContext dctx, Map<String, ? extends Object> context) throws IOException,
		...
		if (UtilValidate.isNotEmpty(context.get("_uploadedFile_fileName"))) {
			...
			String filePathPrefix = tenantId.concat("/catalog/promos/").concat(productPromoId).concat("/").concat(productPromoContentTypeId);
	...
	public static Map<String, Object> uploadImageCategory(DispatchContext dctx, Map<String, ? extends Object> context)
		...
		if (productCategory != null) {
			Map<String, Object> dataResourceCtx = UtilMisc.toMap("userLogin", context.get("userLogin"), "_uploadedFile_fileName", fileName,
					"_uploadedFile_contentType", context.get("_uploadedFile_contentType"), "uploadedFile", context.get("uploadedFile"), "folder",
					tenantId.concat("/catalog/category/").concat(productCategoryId), "public", isPublic);
			...
			String path = (String) result.get("path");
			if (isPublic.equals("Y")) {
				productCategory.put(map.get(fileType), JcrClientProvider.WEB_DAV_URI.concat(JcrClientProvider.WSP_DEFAULT).concat(path));
			} else {
				if (isPublic.equals("N")) {
					productCategory.put(map.get(fileType), JcrClientProvider.WEB_DAV_URI.concat(JcrClientProvider.WSP_SECURITY).concat(path));
				}
			}
			try {
