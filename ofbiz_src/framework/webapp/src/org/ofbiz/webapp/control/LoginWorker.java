    ...
    public static String login(HttpServletRequest request, HttpServletResponse response) {
        ...
        if(UtilValidate.isEmpty(tenantId)) {
        	...
        	if(values != null && !values.isEmpty()) {
        		tenantId = values.get(0).getString("tenantId");
        	}
        }
        ...
        if (ModelService.RESPOND_SUCCESS.equals(result.get(ModelService.RESPONSE_MESSAGE))) {
            ...
            /* add cookie user_id */
            String messsge = doMainLogin(request, response, userLogin, userLoginSession);
            if(messsge.equals("success")) {
            	GenericValue value = null;
            	String iu = "";
            	try {
            		value = delegator.findOne("UserCookie", false, "userId", username);
            	} catch (GenericEntityException e) {
            	...
            	if(value != null) {
            		...
            		iu = delegator.getNextSeqId("UserCookie");
            		value = delegator.makeValue("UserCookie", UtilMisc.toMap("userId", username, "cookie", iu));
            		try {
