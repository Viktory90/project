					 ...
					 { text: '${uiLabelMap.FormFieldTitle_glResourceTypeId}', dataField: 'glResourceTypeId', width: 180, columntype: 'dropdownlist', hidden: true,
              		 ...
              		 { text: '${uiLabelMap.FormFieldTitle_parentGlAccountId}', dataField: 'parentGlAccountId', width: 150, columntype: 'dropdownlist',
					 	createeditor: function (row, column, editor) {
                            ...
                            editor.jqxDropDownList({source: dataAdapterGla, displayMember:\"description\", valueMember: \"glAccountId\"
                        }); 
					 }},
              		 { text: '${uiLabelMap.accountCode}', dataField: 'accountCode', width: 250 },
              		 { text: '${uiLabelMap.accountName}', dataField: 'accountName', width: 300 },
              		 { text: '${uiLabelMap.description}', dataField: 'description', width: 300 },
              		 { text: '${uiLabelMap.FormFieldTitle_postedBalance}', dataField: 'postedBalance', editable:false }"/>
...
<script type="text/javascript" language="Javascript">
    ...
    <#if listTaxFormId?size gt 0>
		...
		<#assign lgTFDes="var lgTFDes = [\"" + StringUtil.wrapString(listTaxFormId.get(0).description?if_exists) + "\""/>
		<#if listTaxFormId?size gt 1>
			<#list 1..(itlength - 1) as i>
				...
				<#assign lgTFDes=lgTFDes + ",\"" + StringUtil.wrapString(listTaxFormId.get(i).description?if_exists) + "\""/>
			</#list>
		</#if>
		<#assign lgTF=lgTF + "];"/>
		<#assign lgTFValue=lgTFValue + "];"/>
		<#assign lgTFDes=lgTFDes + "];"/>
	<#else>
    	<#assign lgTF="var lgTF = [];"/>
    	<#assign lgTFValue="var lgTFValue = [];"/>
		<#assign lgTFDes="var lgTFDes = [];"/>
    </#if>
	${lgTF}
	${lgTFValue}
	${lgTFDes}	
