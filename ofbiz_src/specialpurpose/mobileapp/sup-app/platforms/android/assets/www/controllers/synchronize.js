 ...
 */
olbius.controller('SynchronizeController', function($rootScope, $scope, $controller,$timeout,$location,SynchronizeService, ProductService, SqlService,OrderService,DashboardServices ,CategoryService ,InventoryService ,aSynchronizeService ,CustomerService,CustomerOpinion ,EmployeeService) {
	...
	$scope.initData = function(flag) {
		...
		OrderService.getList($rootScope.showLoading,$rootScope.configPage.index,$rootScope.configPage.pageSize,'thisweek').then(function(data) {
			...
			console.log('data init' + JSON.stringify(data) + $rootScope.configPage.index + $rootScope.configPage.pageSize);
			var input = [];
			var fields = ["orderId","orderDate","grandTotal","groupName","status"];
			...
			setTimeout(function(){
				$rootScope.hideLoading();
				$rootScope.openDialog($rootScope.Map.ReanswerSynchronizeNetwork[$rootScope.key]);
			},10000);
	...
	$scope.showDialog = function(){
		BootstrapDialog.show({
		            ...
		            message: $rootScope.Map.ReanswerSynchronize[$rootScope.key],
		            ...
		            buttons: [{
		                ...
		                action: function(dialogRef){
		                    ...
		                    if($rootScope.network.status){
								...
								dialogRef.getModalBody().html($rootScope.Map.SynchornizeSuccess[$rootScope.key]);
		                    }else {
								$scope.checkSyncComplete.Synchronize = false;
		                    	dialogRef.getModalBody().html($rootScope.Map.ReanswerSynchronizeNetwork[$rootScope.key]);
		                    }
		                    setTimeout(function(){
	                    		dialogRef.close();
		                    },1500);
