...
<script type="text/javascript">
    $(function(){
        ...
        loadPartyOlap(['company'], '2015-01-01', '2015-01-10', "DAY");

        popup._addDropDownList("dateType", "Report", ["DAY", "WEEK", "MONTH", "QUARTER", "YEAR" ], 0);
        popup._addDateTimeInput("from_date", "From", "yyyy-MM-dd", '2015-01-01');
        popup._addDateTimeInput("thru_date", "From", "yyyy-MM-dd", '2015-01-10');

        popup._addOK(function () {
            popup._close();
            loadPartyOlap(popup._getParty(), $("#from_date").val(), $("#thru_date").val(), $("#dateType").val());
        });
    });

    function loadPartyOlap(group, fromdate, thrudate, datetype) {
