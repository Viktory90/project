...
<#list compareList as product>
    ...
    <#-- Brand name -->
    <tr>
        <td class="aligned-color">${uiLabelMap.ProductBrandName}</td>
<#list compareList as product>
    ...
    <#-- Description -->
    <tr>
        <td class="aligned-color">${uiLabelMap.ProductProductDescription}</td>
<#list compareList as product>
    <#assign productData = productDataMap[product.productId]/>
    <#assign productContentWrapper = productData.productContentWrapper/>
    ...
    <#-- Long Description -->
    <tr>
        <td  class="aligned-color">${uiLabelMap.ProductLongDescription}</td>
<#list compareList as product>
    <#assign productData = productDataMap[product.productId]/>
    <#assign productContentWrapper = productData.productContentWrapper/>
                ...
                <form method="post" action="<@ofbizUrl secure="${request.isSecure()?string}">additem</@ofbizUrl>" name="compare2FormAdd${product_index}">
                    ...
                    <div class="input-append">
			            ...
			            <span style="white-space: nowrap;">
				            <button onclick="doPostViaParent('compare2FormAdd${product_index}');" class="btn btn-primary" type="button"><i class="icon-shopping-cart"></i> ${uiLabelMap.OrderAddToCart}
				            </button>
			            </span>
        <#if prodCatMem?exists && prodCatMem.quantity?exists && 0.00 < prodCatMem.quantity?double>
			            <span style="white-space: nowrap;">
				            <button onclick="doPostViaParent('compareFormAddDefault${product_index}');" class="btn btn-primary" type="button"><i class="icon-shopping-cart"></i> ${uiLabelMap.CommonAddDefault}(${prodCatMem.quantity?string.number}) ${uiLabelMap.OrderToCart}
				            </button>
			            </span>
        </#if>
					</div>
