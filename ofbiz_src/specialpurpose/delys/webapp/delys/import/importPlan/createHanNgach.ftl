...
function newButton() {
	setTimeout(function(){ $("#myButton").html("<input type='submit' class='btn btn-primary' value='${uiLabelMap.CreateToPDF}'>"); }, 3000);
}
var count = 0;
...
<table class="table table-striped table-bordered dataTable" cellspacing="0">
	...
	<tr id="dateChoice">
		...
		<td><button class="btn btn-warning" onclick="btnCancelClick();return false;">${uiLabelMap.Cancel}</button></td>
		<td><button class="btn btn-primary" id="btnCreate" onclick="btnCreateClick();return false;anyOneChange();">${uiLabelMap.Create}</button></td>
	</tr>
</table>
...
<div id="myButton">
<input type="submit" class='btn btn-primary' onclick="storeProductToList();return false;" value="${uiLabelMap.Save}">
</div>
</form>