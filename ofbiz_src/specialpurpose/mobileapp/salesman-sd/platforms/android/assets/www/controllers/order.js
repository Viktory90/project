 ...
 * */
olbius.controller('CreateOrderController', function($rootScope, $scope,$route, $controller, $timeout, $location,$compile,$cordovaNetwork, OrderService, ProductService, SqlService, GPS) {
	...
	$scope.btcreateOrder = function(){
		if($scope.products && $scope.products.length > 0){
			...
			bodyOrder += '<th class="center">'+$rootScope.Map.ProductId[$rootScope.key]+'</th>';		
			bodyOrder += '<th class="center">'+$rootScope.Map.ProductName[$rootScope.key]+'</th>';	
			bodyOrder += '<th class="center">'+$rootScope.Map.Price[$rootScope.key]+'</th>';	
			bodyOrder += '<th class="center">'+$rootScope.Map.Quantity[$rootScope.key]+'</th>';	
			var str = "";
			for (var x in $scope.products) {
			...
			bodyOrder += $rootScope.Map.TotalAmount[$rootScope.key]+': <b class="theme-color" >'+ $scope.FormatNumberBy3($scope.calculateTotalCost()) + 'đ</b></div></div></div>';
		}else {
	...
	$scope.submitOrder = function() {
		...
		var renderTable = '<table class="table table-striped table-bordered table-hover products"><thead>';
			renderTable += '<th class="center">'+$rootScope.Map.ProductId[$rootScope.key]+'</th>';		
			renderTable += '<th class="center">'+$rootScope.Map.ProductName[$rootScope.key]+'</th>';	
			renderTable += '<th class="center">'+$rootScope.Map.Quantity[$rootScope.key]+'</th>';	
			renderTable += '<th class="center">'+$rootScope.Map.Price[$rootScope.key]+'</th></thead><tbody><tr>';	
		...
		if($rootScope.network.status){
			...
			GPS.getLocationCallback().then(function(pos){
				if(pos.lat && pos.long){
					positionCreateOrder.latitude = pos.lat;
					positionCreateOrder.longitude = pos.long;
					OrderService.submitOrder(customer, products,positionCreateOrder,$rootScope.showLoading ,$timeout).then(function(data) {
						...
						bodyOrder += '<th class="center">'+$rootScope.Map.Price[$rootScope.key]+'</th>';	
						bodyOrder += '<th class="center">'+$rootScope.Map.Quantity[$rootScope.key]+'</th>';	
						console.log('body order' + bodyOrder);
						var str = "";
						for (var x in $scope.products) {
							var product = $scope.products[x];
							if (product.quantity) {
						...
						if(data.res["OrderInfo"] && data.res["OrderInfo"].length > 0 ){
							bodyOrder += '<div style="height:28px" class="row"><div class="width-80 label label-info label-xlg arrowed-in arrowed-in-right headerPromotions"><h4 class="widget-title col-xs-12 col-md-12 white-color" style="margin : 0 auto 0 auto"><i class="white-color fa fa-list-alt"></i>&nbsp;'+ $rootScope.Map.PromotionDetail[$rootScope.key]  +'</h4></div></div>';	
